...
1. Identify the exact physical revision. Make every version people can
   encounter a separate package with its own stable ID, slug, asset, inventory,
   and paths; never overwrite another revision's identity or saved selection.
...
4. Create the flat `Hangboards/<slug>/board.json` plus
   `assets/primary.png`. Use a clean, simplified, straight-on presentation
