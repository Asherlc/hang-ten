...
If SwiftPM fails before XCTest with a checkout `fatalError`, inspect shared Xcode package repositories read-only first. When the exact `Package.resolved` commit objects are present and pass object-integrity checks, copy or clone only those exact checkouts into workspace-owned local `SourcePackages` and package-cache paths; do not fetch arbitrary revisions or mutate/repair shared caches, and do not skip the native tests. Run the current Xcode interface's equivalent of resolved-file, automatic-update-disabled, locked/offline resolver options (the verified interface used `-clonedSourcePackagesDirPath`, `-packageCachePath`, `-disableAutomaticPackageResolution`, `-onlyUsePackageVersionsFromResolvedFile`, and `-skipPackageUpdates`). Remove the local cache after verification while preserving durable resolver logs, pinned-object checks, test results, and failure evidence.
