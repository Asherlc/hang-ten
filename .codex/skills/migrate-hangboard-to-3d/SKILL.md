...
For recesses or pockets near the body silhouette or a stepped transition, require a complete visible physical wood margin in both front and oblique views. Regression-check that margin against the authored geometry and actual export (for example, with exterior-rim probes or rays); route any failure to Astra for geometry correction, never mask it with camera/lighting or visual-only adjustments. Keep this rule shape-based and do not overfit board-specific numeric values.
