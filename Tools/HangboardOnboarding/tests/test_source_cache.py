...
def test_local_and_http_sources_cache_exact_bytes_and_pixel_hashes(tmp_path: Path, image_server: str):
    local = tmp_path / "input.png"
    local.write_bytes(_png_bytes())
    local_cached = cache_source(str(local), tmp_path / "local-cache")
    http_cached = cache_source(f"{image_server}/image.png", tmp_path / "http-cache")
...
def test_source_json_strips_url_credentials_query_and_fragment(tmp_path: Path, image_server: str):
    source = cache_source(f"{image_server}/redirect?token=secret#fragment", tmp_path / "cache")
...
def test_redirect_must_remain_http_or_https(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    class Response:
        def read(self, _limit):
            return _png_bytes()

        def geturl(self):
            return "file:///private/source.png"
    ...
    monkeypatch.setattr("hangboard_vectorizer.source_cache.urlopen", lambda *_args, **_kwargs: Response())

    with pytest.raises(ValueError, match="remain HTTP"):
        cache_source("https://example.invalid/image.png", tmp_path / "cache")
...
def test_oversized_empty_animated_and_malformed_sources_publish_nothing(tmp_path: Path):
    ...
    Image.new("RGB", (700, 512)).save(animated, save_all=True, append_images=[Image.new("RGB", (700, 512), "red")])
