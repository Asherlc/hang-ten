...
def evaluate_stage4_candidate(
    candidate_root: Path, *, source_png: bytes, stage3_document: Mapping[str, object],
    stage3_acceptance_raw_sha256: str, manual_oracle_path: Path | None,
    browser_oracle_path: Path | None, oracle_paths: Mapping[str, str] | None,
    ...
    if manual_oracle_path is not None:
        ...
        difference = np.abs(source[..., :3].astype(np.int16) - manual[..., :3].astype(np.int16))
    ...
    if browser_oracle_path is not None:
        ...
        generated, generated_mask = _browser_proof(source, browser.shape[1], browser.shape[0])
    ...
    if stage3_acceptance is not None:
        ...
        oracle_geometry = _validate_v14_geometry(candidate, stage3_document, stage3_acceptance, oracle_paths)
        oracle_region_count = oracle_geometry["regionCount"]
    leakage = 0
    generic_accepted = True
    manual_accepted = (
        manual_png_exact is True and manual_rgb_mae is not None and manual_rgb_mae <= 4.0
        and manual_maximum_rgb_error is not None and manual_maximum_rgb_error <= 32
    ) if manual_oracle_path is not None else True
    browser_accepted = (
        product_mask_iou is not None and product_mask_iou >= .995
        and full_canvas_rgb_mae is not None and full_canvas_rgb_mae <= 4.0
        and maximum_rgb_error is not None and maximum_rgb_error <= 32
    ) if browser_oracle_path is not None else True
    accepted = generic_accepted and manual_accepted and browser_accepted and leakage == 0
    review = _review_image(manual, _decode_png(candidate.normal_png, "normal preview"), candidate, manual_rgb_mae, browser_rgb_mae, leakage)
    _encode_png(review, candidate_root / "stage-4-review.png")
    acceptance = {
        "accepted": accepted,
        "artifacts": {"candidateHashes": "stage-4-candidate-hashes.json", "manifest": "stage-4-manifest.json", "product": "stage-4-product.svg", "review": "stage-4-review.png"},
        "candidateHashes": hashes,
        "metrics": {
            "browserRgbMae": browser_rgb_mae, "fullCanvasRgbMae": full_canvas_rgb_mae,
            "highlightLeakagePixels": leakage, "manualPngExact": manual_png_exact,
            "manualRgbMae": manual_rgb_mae, "manualMaximumRgbError": manual_maximum_rgb_error,
            "maximumRgbError": maximum_rgb_error, "productMaskIou": product_mask_iou,
        },
    ...
    if not accepted:
        raise ValueError("Stage 4 candidate does not meet comparison acceptance thresholds")
    return acceptance
...
def _verify_hashes(root: Path) -> dict[str, str]:
    ...
    try:
        document = json.loads((root / "stage-4-candidate-hashes.json").read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        raise ValueError("Stage 4 candidate hash manifest is invalid") from error
    files = document.get("files") if isinstance(document, dict) else None
    if not isinstance(document, dict) or set(document) != {"files", "schemaVersion"} or document.get("schemaVersion") != 1 or not isinstance(files, dict) or set(files) != _CANDIDATE_NAMES:
        raise ValueError("Stage 4 candidate hash manifest is invalid")
    for name, digest in files.items():
        if not isinstance(digest, str) or sha256((root / name).read_bytes()).hexdigest() != digest:
...
def _verify_svg_manifest(root: Path, candidate: Stage4Candidate) -> None:
    try:
        ...
        manifest = json.loads((root / "stage-4-manifest.json").read_text(encoding="utf-8"))
    ...
    if svg.tag != ns + "svg" or svg.get("viewBox") != f"0 0 {candidate.width} {candidate.height}":
    ...
    if not isinstance(href, str) or not href.startswith("data:image/png;base64,") or base64.b64decode(href.split(",", 1)[1], validate=True) != candidate.source_png:
    ...
    for item, path in zip(manifest["regions"], paths, strict=True):
        if (path.get("id"), path.get("data-piece"), path.get("data-type"), path.get("d")) != (item["id"], item["pieceId"], item["type"], item["displayPath"]):
...
def _candidate_bytes(candidate: Stage4Candidate) -> dict[str, bytes]:
    return {
        "stage-4-product.svg": candidate.svg, "stage-4-manifest.json": candidate.manifest,
        "stage-4-normal.png": candidate.normal_png, "stage-4-highlights.json": candidate.highlight_manifest,
        **{f"stage-4-highlight-{name}.png": data for name, data in candidate.highlight_pngs},
    }
...
def _browser_proof(rgba: np.ndarray, width: int, height: int) -> tuple[np.ndarray, np.ndarray]:
    """Place the source on white using the browser's generic contain transform."""
    scale = min(width / rgba.shape[1], height / rgba.shape[0])
    target = Image.fromarray(rgba, mode="RGBA").resize((round(rgba.shape[1] * scale), round(rgba.shape[0] * scale)), Image.Resampling.BICUBIC)
    ...
    product_mask[y:y + target.height, x:x + target.width] = np.asarray(target)[..., 3] > 1
    return np.asarray(canvas.convert("RGB")), product_mask


def _review_image(manual: np.ndarray | None, normal: np.ndarray, candidate: Stage4Candidate, manual_mae: float | None, browser_mae: float | None, leakage: int) -> np.ndarray:
    lookup = dict(candidate.highlight_pngs)
    panels = [manual if manual is not None else normal, normal, *(_decode_png(lookup[name], name) for name in ("jugs", "slopers", "symmetric-pockets", "mixed"))]
    height, width = normal.shape[:2]
    label_height, footer = 16, 16
    canvas = Image.new("RGB", (width * 2, (height + label_height) * 3 + footer), "white")
    draw = ImageDraw.Draw(canvas)
    font = ImageFont.load_default()
    labels = ("manual normal", "automatic normal", "jugs", "slopers", "symmetric pockets", "mixed")
    for index, (label, rgba) in enumerate(zip(labels, panels, strict=True)):
        ...
        image = Image.fromarray(rgba, mode="RGBA") if rgba.shape[2] == 4 else Image.fromarray(rgba, mode="RGB")
        canvas.paste(image, (x, y + label_height), image if image.mode == "RGBA" else None)
    ...
    draw.text((2, canvas.height - footer + 2), f"normal MAE: {manual_text}; browser MAE: {browser_text}; highlight leakage: {leakage}", fill=(25, 25, 25), font=font)
...
def _opaque_rgb(image: np.ndarray) -> np.ndarray:
    ...
    return np.rint(image[..., :3].astype(np.float32) * alpha + 255.0 * (1.0 - alpha)).astype(np.uint8)


def _validate_v14_geometry(candidate: Stage4Candidate, document: Mapping[str, object], acceptance: Mapping[str, object], oracle_paths: Mapping[str, str]) -> dict[str, object]:
    ...
    if (not isinstance(regions, list) or len(regions) != 22 or not isinstance(pairing, list)
            or not isinstance(expected_regions, list) or not isinstance(expected_metrics, dict)
            or not isinstance(raw_regions, list) or not isinstance(raw_metrics, dict)):
    ...
    if len(region_ids) != 22 or len(set(region_ids)) != 22 or {kind: types.count(kind) for kind in ("pocket", "sloper", "jug")} != {"pocket": 17, "sloper": 3, "jug": 2}:
        raise ValueError("Stage 4 V14 topology is invalid")
    pairs = [(item.get("candidateId"), item.get("oracleId")) for item in pairing if isinstance(item, dict)]
    if (len(pairs) != 22 or len(set(pairs)) != 22 or {first for first, _ in pairs} != set(region_ids)
            or {second for _, second in pairs} != set(oracle_paths) or len(oracle_paths) != 22):
        raise ValueError("Stage 4 V14 pairing is invalid")
    center = [second for first, second in pairs if next(item["type"] for item in regions if item["id"] == first) == "sloper" and second == "sloper-center"]
    if center != ["sloper-center"]:
        raise ValueError("Stage 4 V14 center sloper is not continuous")
    paths = []
    for region in regions:
        assert isinstance(region, dict)
        parsed = parse_display_path(region.get("displayPath"), str(region.get("id")), candidate.width, candidate.height, allow_linear_segments=True)
        if parsed is None:
            raise ValueError("Stage 4 geometry path is missing")
        paths.append(parsed)
    from .stage2_evaluation import _oracle_masks, _region_metrics, _summary
    masks = dict(zip(region_ids, rasterize_paths_half_open(tuple(paths), candidate.width, candidate.height), strict=True))
    oracle = _oracle_masks(oracle_paths, candidate.width, candidate.height)
    actual_regions = [_region_metrics(first, second, masks[first], oracle[second][1]) for first, second in pairs]
    actual_metrics = _summary(actual_regions)
    if actual_regions != expected_regions or actual_metrics != expected_metrics:
        raise ValueError("Stage 4 geometry metrics do not exactly inherit accepted Stage 3")
    return {"oracleMetrics": expected_metrics, "rawMetrics": raw_metrics, "rawRegions": raw_regions, "regionCount": 22}
