...
def compose_stage4(source_png: bytes, stage3_document: Mapping[str, object]) -> Stage4Candidate:
    ...
    manifest = {
        ...
        "regions": [
            {"displayPath": region.display_path.data, "id": region.id,
             "pieceId": region.piece_id, "pieceIndex": region.piece_index,
    ...
    return Stage4Candidate(
        width, height, source_png, _serialize_svg(source_png, width, height, regions),
        _json_bytes(manifest), _encode_png(rgba), tuple(previews), _json_bytes(highlights),
    )


def select_highlight_scenarios(
    regions: tuple[Stage4Region, ...], masks: Mapping[str, np.ndarray], width: int, height: int
    ...
    seeds: list[str] = []
    if jugs:
        seeds.append(min(jugs, key=lambda identifier: (metrics[identifier][0], identifier)))
    if slopers:
        seeds.append(max(slopers, key=lambda identifier: (metrics[identifier][3], identifier)))
    if pockets:
        seeds.append(min((region.id for region in pockets), key=lambda identifier: (
            (metrics[identifier][0] - width / 2) ** 2 + (metrics[identifier][1] - height / 2) ** 2,
            identifier,
        )))
    ...
    while len(mixed) < min(6, len(regions)):
        ...
        def distance(identifier: str) -> tuple[float, str]:
            ...
            nearest = min((x - metrics[chosen][0]) ** 2 + (y - metrics[chosen][1]) ** 2 for chosen in mixed) if mixed else float("inf")
            return nearest, identifier
        mixed.append(max(candidates, key=distance))
    return {"jugs": jugs, "slopers": slopers, "symmetric-pockets": symmetric,
            "mixed": tuple(mixed), "all": all_ids}


def render_highlight(
    rgba: np.ndarray, masks: Mapping[str, np.ndarray], selected_ids: tuple[str, ...], *,
    color: tuple[int, int, int] = (255, 90, 31), opacity: float = 0.48,
) -> np.ndarray:
    ...
    if not 0 <= opacity <= 1 or len(color) != 3 or any(type(value) is not int or not 0 <= value <= 255 for value in color):
        raise ValueError("highlight color or opacity is invalid")
    if len(set(selected_ids)) != len(selected_ids) or any(identifier not in masks for identifier in selected_ids):
        raise ValueError("highlight selection references unknown regions")
    coverage = _selected_union(masks, selected_ids, source.shape[0], source.shape[1]).astype(np.float32)
    alpha = coverage * opacity
    result = source.copy()
    rgb = source[..., :3].astype(np.float32)
    overlay = np.asarray(color, dtype=np.float32)
    result[..., :3] = np.clip(np.rint(rgb * (1.0 - alpha[..., None]) + overlay * alpha[..., None]), 0, 255).astype(np.uint8)
    return result


def _parse_stage3_regions(document: Mapping[str, object], width: int, height: int) -> tuple[Stage4Region, ...]:
    expected = {"canonicalSeams", "height", "pieces", "profile", "regions", "schemaVersion", "stage", "stage2AcceptanceRawSha256", "width"}
    if not isinstance(document, Mapping) or set(document) != expected or document.get("schemaVersion") != 1 or document.get("stage") != 3 or document.get("width") != width or document.get("height") != height:
    ...
    for index, piece in enumerate(pieces):
        if not isinstance(piece, Mapping) or piece.get("pieceIndex") != index or piece.get("id") != f"piece-{index + 1:02d}-silhouette":
    ...
    for ordinal, value in enumerate(values, start=1):
        ...
        identifier, piece_index, grip_type = value.get("id"), value.get("pieceIndex"), value.get("type")
        if (not isinstance(identifier, str) or identifier in seen or type(piece_index) is not int
                or piece_index not in owners or grip_type not in _TYPES
        ...
        parsed_path = parse_display_path(raw_path, identifier, width, height, allow_linear_segments=True)
        ...
        result.append(Stage4Region(identifier, owners[piece_index], piece_index, grip_type, DisplayPath(raw_path, parsed_path.commands, parsed_path.coordinates)))
    return tuple(result)


def _coverage_masks(regions: tuple[Stage4Region, ...], width: int, height: int) -> dict[str, np.ndarray]:
    masks: dict[str, np.ndarray] = {}
    for region in regions:
        parsed = parse_display_path(region.display_path.data, region.id, width, height, allow_linear_segments=True)
        assert parsed is not None
        high = rasterize_display_path(parsed, width, height, scale=4)
        masks[region.id] = high.reshape(height, 4, width, 4).mean(axis=(1, 3), dtype=np.float64).astype(np.float32)
    return masks
...
def _symmetric_pockets(pockets: tuple[Stage4Region, ...], metrics: Mapping[str, tuple[float, float, float, float, float]], width: int, height: int) -> tuple[str, ...]:
    if len(pockets) < 2:
        return ()
    ...
    for region in ordered:
        if not rows or metrics[region.id][1] - metrics[rows[-1][-1].id][1] > .35 * median_height:
            rows.append([region])
        else:
            rows[-1].append(region)
    row = min(rows, key=lambda values: (abs(np.mean([metrics[item.id][1] for item in values]) - height / 2), tuple(item.id for item in values)))
    midpoint = width / 2
    pairs: list[tuple[tuple[float, float, float, tuple[str, str]], tuple[str, str]]] = []
    for left in row:
        for right in row:
            if metrics[left.id][0] >= midpoint or metrics[right.id][0] <= midpoint:
                continue
            lx, ly, _, _, la = metrics[left.id]; rx, ry, _, _, ra = metrics[right.id]
            score = (abs(ly - ry) / max(median_height, 1), abs((lx + rx) - width) / max(width, 1), abs(la - ra) / max(la, ra, 1), tuple(sorted((left.id, right.id))))
...
def _selected_union(masks: Mapping[str, np.ndarray], selected_ids: tuple[str, ...], height: int, width: int) -> np.ndarray:
...
def _validate_highlight(source: np.ndarray, highlighted: np.ndarray, masks: Mapping[str, np.ndarray], selected: tuple[str, ...], name: str) -> None:
    changed = np.any(source[..., :3] != highlighted[..., :3], axis=2)
    union = _selected_union(masks, selected, source.shape[0], source.shape[1]) > 0
    unrelated = _selected_union(masks, tuple(identifier for identifier in masks if identifier not in selected), source.shape[0], source.shape[1]) > 0
    ...
    if selected:
        ...
        if denominator == 0 or np.count_nonzero(changed & union) / denominator < .995:
...
def _encode_png(rgba: np.ndarray) -> bytes:
    output = BytesIO()
    Image.fromarray(np.asarray(rgba, dtype=np.uint8), mode="RGBA").save(output, format="PNG", optimize=False, compress_level=9)
    return output.getvalue()


def _serialize_svg(source_png: bytes, width: int, height: int, regions: tuple[Stage4Region, ...]) -> bytes:
    ET.register_namespace("", "http://www.w3.org/2000/svg")
    root = ET.Element("{http://www.w3.org/2000/svg}svg", {"width": str(width), "height": str(height), "viewBox": f"0 0 {width} {height}"})
    ...
    ET.SubElement(root, "{http://www.w3.org/2000/svg}image", {"class": "product-render", "x": "0", "y": "0", "width": str(width), "height": str(height), "preserveAspectRatio": "none", "href": "data:image/png;base64," + base64.b64encode(source_png).decode("ascii")})
    for region in regions:
        ET.SubElement(root, "{http://www.w3.org/2000/svg}path", {"id": region.id, "class": "grip-region", "data-piece": region.piece_id, "data-type": region.grip_type, "d": region.display_path.data})
