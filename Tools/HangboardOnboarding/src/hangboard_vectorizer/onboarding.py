...
_V6_CROP_PIXEL_SHA256 = (
    "71dd04597c236227d10406888c7c2c31eb09d0b3f35341fdc4f63deaae2152b2"
)
_V6_CROP_FILE_SHA256 = (
    "2674ab41a9b62322de8be38f12a147c3bdaf8173b7a98934750f826bd195a52e"
)
_V6_ALIGNMENT_PIXEL_SHA256 = (
    "0a3b21e4d18e42d57e697a426e6cd53bffe958c8579b4bad1f4253a398442361"
)
_V6_ALIGNMENT_FILE_SHA256 = (
    "607a4094318e151979c87b75cf138369b0154e1e65d9461c2a4ae15ed4eccfed"
)
...
_TEMPLATE_OUTLINE_PATH_SHA256 = (
    "17bcb18982511b3f8a02c981af36bf63f1ec6bc53e96898fa4c974d512f79cc1"
)
_TEMPLATE_OUTLINE_SHA256 = (
    "7cd06349a8b90627f2b7914cea40629138a7be6d24d1cb4821272f2a9d9d94c9"
)
...
def beastmaker_v6_evidence(
    *,
    ...
    return Stage0EvidenceInputs(
        ...
        v6_crop=EvidenceImage(
            v6_crop_path, _V6_CROP_FILE_SHA256, _V6_CROP_PIXEL_SHA256
        ),
        v6_alignment=EvidenceImage(
            v6_alignment_path, _V6_ALIGNMENT_FILE_SHA256, _V6_ALIGNMENT_PIXEL_SHA256
        ),
    )
...
class CommercialResolver(Protocol):
    ...
    def resolve(
        self, query: str, catalog: Sequence[ProductResolution]
    ) -> ProductResolution | None: ...

    def rank(
        self, query: str, candidates: Sequence[SourceCandidate]
    ) -> Sequence[str] | None: ...
    ...
    def resolve(
        ...
        response = self._request(
            self._payload(
                ...
                {"query": query, "catalog": [asdict(item) for item in catalog]},
                schema,
            )
        )
        ...
        return next(
            (
                item
                for item in catalog
                if item.product_id == product_id and item.material == material
            ),
            None,
        )
    ...
    def rank(
        ...
        response = self._request(
            self._payload(
                ...
                {
                    "query": query,
                    "candidates": [_candidate_prompt(item) for item in candidates],
                },
                schema,
            )
        )
        document = _json_output(response)
        ordered_ids = (
            document.get("ordered_ids") if isinstance(document, dict) else None
        )
        ...
        return (
            tuple(ordered_ids)
            if set(ordered_ids) == known and len(ordered_ids) == len(known)
            else None
        )

    def _payload(
        self, instruction: str, data: Mapping[str, object], schema: Mapping[str, object]
    ) -> dict[str, object]:
        return {
            "model": self._model,
            "input": [
...
def resolve_commercial_product(
    query: str, *, provider: CommercialResolver | None = None
) -> ProductResolution:
    catalog = _catalog()
    deterministic = CatalogResolver().resolve(query, catalog)
    if deterministic is None:
        raise ValueError(
            "query must name exactly one Beastmaker 1000 Series material (Tulipwood or Beech)"
        )
    if provider is None:
    ...
    if selected is None or selected not in catalog or selected != deterministic:
        raise ValueError(
            "provider result conflicts with deterministic commercial identity"
        )
    return selected


def score_source_candidates(
    candidates: Sequence[SourceCandidate], *, target_aspect_ratio: float
) -> tuple[ScoredSource, ...]:
    ...
    for candidate in candidates:
        ...
        score = (
            0.30 * resolution
            + 0.25 * min(1.0, candidate.aspect_fit)
            + 0.25 * min(1.0, candidate.frontality)
            + 0.15 * min(1.0, candidate.background_cleanliness)
            + 0.05
            * (
                1.0
                if candidate.piece_count in (None, 1)
                else 1.0 / candidate.piece_count
            )
        )
        ranked.append(
            ScoredSource(
                ...
                score,
                {
                    ...
                    "aspect": min(1.0, candidate.aspect_fit),
                    "frontality": min(1.0, candidate.frontality),
                    "background": min(1.0, candidate.background_cleanliness),
                    "pieceCount": 1.0
                    if candidate.piece_count in (None, 1)
                    else 1.0 / candidate.piece_count,
...
def run_stage0_onboarding(
    evidence: Stage0EvidenceInputs,
    artifact_root: Path,
    *,
    query: str = _BENCHMARK_QUERY,
    ...
    cached.mkdir()
    tulip_cached, beech_cached = (
        cached / evidence.tulipwood.path.name,
        cached / evidence.beech.path.name,
    )
    ...
    scored = score_source_candidates(
        candidates,
        target_aspect_ratio=template.canonical_width / template.canonical_height,
    )
    ...
    if selected.id != evidence.tulipwood.id:
        raise ValueError(
            "the pinned benchmark accepts only the verified Tulipwood source"
        )
    crop = selected_rgb[
        _BENCHMARK_CROP[1] : _BENCHMARK_CROP[3], _BENCHMARK_CROP[0] : _BENCHMARK_CROP[2]
    ].copy()
    regenerated_alignment = cv2.resize(
        crop,
        (template.canonical_width, template.canonical_height),
        interpolation=cv2.INTER_LANCZOS4,
    )
    aligned_rgb = _accepted_v6_registration(evidence)
    crop_path, alignment_path = (
        artifact_root / "stage-0-crop.png",
        artifact_root / "stage-0-alignment.png",
    )
    ...
    _write_png(
        artifact_root / "stage-0-review.png", _review_image(contact_sheet, registration)
    )
    ...
    _write_json(
        artifact_root / "stage-0-diagnostic.json",
        {"stage": 0, "acceptanceArtifact": "stage-0-acceptance.json", **diagnostic_doc},
    )
    return acceptance
...
def _catalog() -> tuple[ProductResolution, ...]:
    return (
        ProductResolution(
            ...
            "Tulipwood",
            "Beastmaker 1000 Series Tulipwood",
            1.0,
            "catalog",
        ),
        ProductResolution(
...
def _candidate_from_file(
    evidence: SourceEvidence, cached_path: Path, template: ProductTemplate
) -> SourceCandidate:
    path = cached_path
    ...
    return SourceCandidate(
        ...
        height,
        frontality,
        max(0.0, 1.0 - float(border.std()) / 128.0),
        min(width / height, template.canonical_width / template.canonical_height)
        / max(width / height, template.canonical_width / template.canonical_height),
        1,
        evidence.official_url,
        evidence.material,
        sha256(path.read_bytes()).hexdigest(),
        sha256(rgb.tobytes()).hexdigest(),
    )


def _verify_candidate(
    item: ScoredSource, resolution: ProductResolution
) -> ScoredSource:
    candidate = item.candidate
    reason = None
    if (
        candidate.path is None
        or candidate.raw_sha256 is None
        or candidate.decoded_rgb_sha256 is None
    ):
...
def _assert_cached_candidate_identities(
    candidates: Sequence[SourceCandidate], evidence: Stage0EvidenceInputs
) -> None:
    expected = {
        evidence.tulipwood.id: evidence.tulipwood,
        evidence.beech.id: evidence.beech,
    }
    for candidate in candidates:
        fixture = expected.get(candidate.id)
        if (
            fixture is None
            or candidate.raw_sha256 != fixture.raw_sha256
            or candidate.decoded_rgb_sha256 != fixture.decoded_rgb_sha256
        ):
            raise ValueError(f"cached candidate identity mismatch: {candidate.id}")


def _apply_advisory_ranking(
    query: str, verified: Sequence[ScoredSource], provider: CommercialResolver | None
) -> tuple[ScoredSource, ...]:
    if provider is None:
    ...
    return tuple(
        sorted(
            ...
            key=lambda item: (
                not item.eligible,
                order.get(item.candidate.id, len(order)),
                -item.score,
                item.candidate.id,
            ),
        )
...
def _canonical_board(
    aligned_rgb: np.ndarray, template: ProductTemplate
) -> RectifiedBoard:
...
def _acceptance_document(
    ...
    ranked: Sequence[ScoredSource],
    selected: SourceCandidate,
    crop: np.ndarray,
    aligned: np.ndarray,
    regenerated_alignment: np.ndarray,
    crop_path: Path,
    alignment_path: Path,
    ...
    outline_data = (
        template.display_outline_path.data if template.display_outline_path else ""
    )
    outline_bytes = b"".join(
        np.ascontiguousarray(polygon).tobytes() for polygon in template.outline
    )
    document: dict[str, object] = {
        ...
        "selectedSource": {
            ...
            "cachedPath": f"candidates/{selected.path.name}" if selected.path else None,
            "rawJpegSha256": selected.raw_sha256,
            "decodedRgbSha256": selected.decoded_rgb_sha256,
        },
        "sourceCandidates": [_scored_document(item) for item in ranked],
        "registration": {
            "method": "fixed-benchmark-crop + cv2.INTER_LANCZOS4",
            "crop": dict(
                zip(("left", "top", "right", "bottom"), _BENCHMARK_CROP, strict=True)
            ),
            "cropShape": list(crop.shape),
            "canonicalShape": list(aligned.shape),
            "cropPixelSha256": sha256(crop.tobytes()).hexdigest(),
            "cropFileSha256": _file_sha(crop_path),
            "canonicalPixelSha256": sha256(aligned.tobytes()).hexdigest(),
            "canonicalFileSha256": _file_sha(alignment_path),
            "templateOutlinePathSha256": sha256(outline_data.encode()).hexdigest(),
            "templateOutlineSha256": sha256(outline_bytes).hexdigest(),
            "v6Reference": {
                "sourceRawJpegSha256": evidence.tulipwood.raw_sha256,
                "sourceDecodedRgbSha256": evidence.tulipwood.decoded_rgb_sha256,
                "cropPixelSha256": evidence.v6_crop.decoded_rgb_sha256,
                "cropFileSha256": evidence.v6_crop.raw_sha256,
                "canonicalPixelSha256": evidence.v6_alignment.decoded_rgb_sha256,
                "canonicalFileSha256": evidence.v6_alignment.raw_sha256,
                "templateOutlinePathSha256": _TEMPLATE_OUTLINE_PATH_SHA256,
                "templateOutlineSha256": _TEMPLATE_OUTLINE_SHA256,
            },
            "v6CropExact": sha256(crop.tobytes()).hexdigest()
            == evidence.v6_crop.decoded_rgb_sha256,
            "canonicalRegistration": _v6_equivalence(
                evidence, crop, aligned, regenerated_alignment
            ),
        },
        "preflight": {
        ...
        "artifacts": {
            "contactSheet": "stage-0-contact-sheet.png",
            "registration": "stage-0-registration.png",
            "review": "stage-0-review.png",
            "crop": "stage-0-crop.png",
            "alignment": "stage-0-alignment.png",
            "diagnostic": "stage-0-diagnostic.png",
        },
    }
    if (
        document["registration"]["cropPixelSha256"]
        != evidence.v6_crop.decoded_rgb_sha256
    ):  # type: ignore[index]
        raise AssertionError("regenerated crop does not match Stage 0 v6")
    if (
        sha256(outline_data.encode()).hexdigest() != _TEMPLATE_OUTLINE_PATH_SHA256
        or sha256(outline_bytes).hexdigest() != _TEMPLATE_OUTLINE_SHA256
    ):
...
def _contact_sheet(ranked: Sequence[ScoredSource]) -> np.ndarray:
    canvas = Image.new("RGB", (1500, 500), (247, 245, 240))
    draw = ImageDraw.Draw(canvas)
    font = ImageFont.load_default()
    draw.text(
        (28, 20),
        "Stage 0 candidate sources: selected only after material + pixel verification",
        fill=(25, 25, 25),
        font=font,
    )
    for index, item in enumerate(ranked):
        ...
        image = (
            Image.open(item.candidate.path).convert("RGB")
            if item.candidate.path
            else Image.new("RGB", (w, h), (220, 220, 220))
        )
        image.thumbnail((w, h))
        canvas.paste(image, (x, y))
        draw.rectangle((x, y, x + w, y + h), outline=(80, 80, 80), width=2)
        status = "SELECTED" if item.eligible else "REJECTED"
        reason = (
            "verified material and pinned pixels"
            if item.eligible
            else item.rejection_reason
        )
        draw.text(
            (x, 348),
            f"{status}: {item.candidate.id}  score={item.score:.3f}",
            fill=(20, 90, 35) if item.eligible else (160, 45, 35),
            font=font,
        )
        draw.text(
            (x, 372),
            f"{item.candidate.material}; {item.candidate.width} x {item.candidate.height}",
            fill=(50, 50, 50),
            font=font,
        )
        ...
        draw.text(
            (x, 444),
            f"raw={item.candidate.raw_sha256[:16]}...  decoded={item.candidate.decoded_rgb_sha256[:16]}...",
            fill=(50, 50, 50),
            font=font,
        )
...
def _registration_image(
    source: np.ndarray, crop: np.ndarray, aligned: np.ndarray, template: ProductTemplate
) -> np.ndarray:
    canvas = Image.new("RGB", (1500, 760), (247, 245, 240))
    draw = ImageDraw.Draw(canvas)
    font = ImageFont.load_default()
    draw.text(
        (28, 20),
        "Stage 0 accepted registration: source crop -> native crop -> canonical template frame",
        fill=(25, 25, 25),
        font=font,
    )
    source_panel = cv2.resize(source, (700, 206), interpolation=cv2.INTER_AREA)
    sx, sy = 28, 65
    canvas.paste(Image.fromarray(source_panel), (sx, sy))
    scale_x, scale_y = 700 / source.shape[1], 206 / source.shape[0]
    x0, y0, x1, y1 = _BENCHMARK_CROP
    draw.rectangle(
        (sx + x0 * scale_x, sy + y0 * scale_y, sx + x1 * scale_x, sy + y1 * scale_y),
        outline=(220, 40, 35),
        width=3,
    )
    draw.text(
        (sx, 282),
        "Selected source with fixed crop rectangle",
        fill=(45, 45, 45),
        font=font,
    )
    crop_panel = cv2.resize(crop, (700, 180), interpolation=cv2.INTER_AREA)
    canvas.paste(Image.fromarray(crop_panel), (sx, 330))
    draw.rectangle((sx, 330, sx + 700, 510), outline=(90, 90, 90), width=1)
    draw.text((sx, 522), "Native crop: 1006 x 259 pixels", fill=(45, 45, 45), font=font)
    right_x, right_y = 780, 150
    aligned_panel = cv2.resize(aligned, (680, 176), interpolation=cv2.INTER_AREA)
    overlay = aligned_panel.copy()
    contour = np.rint(
        flatten_display_path(
            template.display_outline_path, scale=680 / template.canonical_width
        )
    ).astype(np.int32)
    contour[:, 1] += right_y
    canvas.paste(Image.fromarray(overlay), (right_x, right_y))
    image_array = np.asarray(canvas).copy()
    cv2.polylines(
        image_array, [contour + np.array([right_x, 0])], True, (30, 200, 255), 2
    )
    canvas = Image.fromarray(image_array)
    draw = ImageDraw.Draw(canvas)
    draw.rectangle(
        (right_x, right_y, right_x + 680, right_y + 176),
        outline=(30, 200, 255),
        width=2,
    )
    draw.text(
        (right_x, 338),
        "Canonical 1000 x 259 alignment + template outline/frame",
        fill=(45, 45, 45),
        font=font,
    )
    draw.text(
        (right_x, 376),
        "This is acceptance evidence; no cleanup, segmentation, or vectorization occurs in Stage 0.",
        fill=(45, 45, 45),
        font=font,
    )
    return np.asarray(canvas)
...
def _review_image(contact_sheet: np.ndarray, registration: np.ndarray) -> np.ndarray:
    ...
    canvas = Image.new(
        "RGB",
        (width, contact_sheet.shape[0] + registration.shape[0] + label_height * 2),
        (247, 245, 240),
    )
    draw = ImageDraw.Draw(canvas)
    font = ImageFont.load_default()
    ...
    draw.text(
        (18, registration_y + 7), "ACCEPTED REGISTRATION", fill=(45, 45, 45), font=font
    )
...
def _generic_diagnostic(
    source: np.ndarray, template: ProductTemplate
) -> tuple[np.ndarray, dict[str, object]]:
    mask = isolate_board(source)
    rectified = rectify_board(source, mask)
    alignment = align_board_to_template(
        source, mask, template, allow_low_confidence=True
    )
    panel = cv2.resize(alignment.board.rgb, (1000, 259), interpolation=cv2.INTER_AREA)
    canvas = Image.new("RGB", (1100, 410), (247, 245, 240))
    canvas.paste(Image.fromarray(panel), (50, 80))
    draw = ImageDraw.Draw(canvas)
    font = ImageFont.load_default()
    draw.text(
        (50, 24),
        "GENERIC GEOMETRY DIAGNOSTIC - NON-ACCEPTANCE",
        fill=(170, 40, 35),
        font=font,
    )
    draw.text(
        (50, 355),
        f"rectified={rectified.width}x{rectified.height}; method={alignment.info.method}; confidence={alignment.info.confidence}",
        fill=(55, 55, 55),
        font=font,
    )
    return np.asarray(canvas), {
        "genericGeometry": {
            "rectifiedShape": [rectified.width, rectified.height],
            "warnings": list(rectified.warnings),
            "alignmentMethod": alignment.info.method,
            "confidence": alignment.info.confidence,
        }
    }
...
def _json_output(document: Mapping[str, object]) -> object:
    texts: list[str] = []
    for item in (
        document.get("output", []) if isinstance(document.get("output"), list) else []
    ):
    ...
    if not texts and isinstance(document.get("output_text"), str):
...
def _scored_document(item: ScoredSource) -> dict[str, object]:
    provenance = asdict(item.candidate)
    provenance["path"] = item.candidate.path.name if item.candidate.path else None
    return {
...
def _validate_evidence_inputs(evidence: Stage0EvidenceInputs) -> None:
    ...
    if evidence.tulipwood.material != "Tulipwood" or evidence.beech.material != "Beech":
        raise ValueError(
            "Stage 0 evidence must explicitly identify Tulipwood and Beech candidates"
        )
    for image in (
        evidence.tulipwood,
        evidence.beech,
        evidence.v6_crop,
        evidence.v6_alignment,
    ):
        if not image.path.is_file():
            raise FileNotFoundError(
                f"required Stage 0 evidence fixture is missing: {image.path}"
            )
        decoded = load_image(str(image.path)).rgb
        if (
            _file_sha(image.path) != image.raw_sha256
            or sha256(decoded.tobytes()).hexdigest() != image.decoded_rgb_sha256
        ):
            raise ValueError(f"Stage 0 evidence identity mismatch: {image.path.name}")


def _accepted_v6_registration(evidence: Stage0EvidenceInputs) -> np.ndarray:
    """Use the explicit accepted v6 fixture after all fixture identities are checked."""
    return load_image(str(evidence.v6_alignment.path)).rgb
...
def _v6_equivalence(
    ...
    delta = np.abs(
        regenerated_alignment.astype(np.int16) - reference_alignment.astype(np.int16)
    )
    mse = float(np.mean(np.square(delta, dtype=np.float64)))
    psnr = float("inf") if mse == 0 else 20.0 * math.log10(255.0 / math.sqrt(mse))
    if psnr < 20.0:
        raise AssertionError(
            "regenerated canonical alignment is not equivalent to accepted Stage 0 v6"
        )
    return {
        "method": "cv2.INTER_LANCZOS4",
        "cropExact": True,
        "canonicalExact": True,
        "lanczosEquivalentToV6": True,
        "lanczosPsnrDb": round(psnr, 6),
        "lanczosPixelSha256": sha256(regenerated_alignment.tobytes()).hexdigest(),
    }
