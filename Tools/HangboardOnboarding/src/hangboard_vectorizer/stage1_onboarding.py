...
def beastmaker_stage1_evidence(
    *, stage0_acceptance_path: Path, source_path: Path, manual_rgba_path: Path,
    manual_review_path: Path, packaged_render_path: Path,
) -> Stage1EvidenceInputs:
    """Construct explicit paths for the approved Beastmaker policy."""
    return Stage1EvidenceInputs(
        stage0_acceptance_path, source_path, manual_rgba_path, manual_review_path,
        packaged_render_path,
    )


def run_stage1_onboarding(
    evidence: Stage1EvidenceInputs, artifact_root: Path, *, profile: CleanupProfile,
) -> Mapping[str, object]:
    ...
    stage0, source, manual_rgba, manual_review, packaged = _validate_inputs(evidence, policy)
    ...
    _require_exact_accepted_output(first, manual_rgba.pixels, manual_review.pixels, packaged.pixels)

    artifact_root.parent.mkdir(parents=True, exist_ok=True)
    temporary = Path(mkdtemp(prefix=f".{artifact_root.name}.", dir=artifact_root.parent))
    try:
        document = _write_artifacts(
            temporary, evidence, policy, stage0, source, first, manual_rgba, manual_review, packaged,
        )
        if artifact_root.exists():
            raise FileExistsError(f"stage 1 artifact root already exists: {artifact_root}")
...
def _validate_inputs(
    evidence: Stage1EvidenceInputs, policy: TrustedStage1Policy,
) -> tuple[Mapping[str, object], _ValidatedImage, _ValidatedImage, _ValidatedImage, _ValidatedImage]:
    ...
    manual_rgba = _read_image(evidence.manual_rgba_path, policy.manual_rgba, "manual RGBA")
    manual_review = _read_image(evidence.manual_review_path, policy.manual_review, "manual review")
    packaged = _read_image(evidence.packaged_render_path, policy.packaged_render, "packaged render")
...
def _validate_profile(profile: CleanupProfile) -> TrustedStage1Policy:
    ...
    if policy is None or (
        profile.product_id, profile.material, profile.piece_count
    ) != (policy.product_id, policy.material, policy.piece_count):
        raise ValueError("unsupported Stage 1 cleanup profile identity/material/piece count")
    return policy


def _load_stage0_acceptance(path: Path, policy: TrustedStage1Policy) -> Mapping[str, object]:
...
def _read_image(path: Path, identity: TrustedImageIdentity, name: str) -> _ValidatedImage:
...
def _validate_stage0_contract(
    stage0: Mapping[str, object], policy: TrustedStage1Policy, source: _ValidatedImage,
) -> None:
    ...
    if resolution.get("product_id") != policy.product_id or resolution.get("material") != policy.material:
    ...
    if (
        ...
        raise ValueError("Stage 0 selected source identity does not match supplied evidence")
    ...
    if tuple(crop.get(key) for key in ("left", "top", "right", "bottom")) != policy.registration_crop:
        raise ValueError("Stage 0 registration crop is not the approved Beastmaker crop")
...
def _validate_profile_result(result: SimplifiedRaster, policy: TrustedStage1Policy) -> SimplifiedRaster:
    if result.rgba.shape != policy.manual_rgba.shape or result.review_rgb.shape != policy.manual_review.shape:
    ...
    if not _transparent_rgb_is_zero(result.rgba):
        raise ValueError("cleanup profile output has nonzero RGB beneath transparent pixels")
    return result
...
def _same_raster(first: SimplifiedRaster, second: SimplifiedRaster) -> bool:
    return all(np.array_equal(a, b) for a, b in zip(
        ...
        (second.rgba, second.review_rgb, second.repair_mask), strict=True,
    ))


def _require_exact_accepted_output(
    result: SimplifiedRaster, manual_rgba: np.ndarray, manual_review: np.ndarray,
    packaged_rgba: np.ndarray,
) -> None:
    if not np.array_equal(result.rgba, manual_rgba) or not np.array_equal(result.review_rgb, manual_review):
        raise ValueError("cleanup profile output is not pixel-exact to the accepted manual output")
    if not np.array_equal(result.rgba, packaged_rgba):
        raise ValueError("cleanup profile output is not pixel-exact to the packaged render")


def _write_artifacts(
    root: Path, evidence: Stage1EvidenceInputs, policy: TrustedStage1Policy,
    stage0: Mapping[str, object], source: _ValidatedImage, result: SimplifiedRaster, manual_rgba: _ValidatedImage,
    manual_review: _ValidatedImage, packaged: _ValidatedImage,
) -> dict[str, object]:
    ...
    _write_accepted_bytes(auto_review, result.review_rgb, manual_review.raw_bytes, "RGB")
    _save_png(root / "stage-1-auto-vs-manual.png", _side_by_side(result.review_rgb, manual_review.pixels))
    ...
    _save_png(root / "stage-1-review.png", _combined_review(source.pixels, result.repair_mask, result.review_rgb, manual_review.pixels, heatmap))
    document = _acceptance_document(
        evidence, policy, stage0, result, manual_rgba, manual_review, packaged, root,
    )
...
def _acceptance_document(
    evidence: Stage1EvidenceInputs, policy: TrustedStage1Policy,
    stage0: Mapping[str, object], result: SimplifiedRaster, manual_rgba: _ValidatedImage,
    manual_review: _ValidatedImage, packaged: _ValidatedImage, root: Path,
) -> dict[str, object]:
    metrics = _comparison_metrics(result.rgba, manual_rgba, result.review_rgb, manual_review, packaged, root)
    mandatory = (
        metrics["automatedEqualsManual"], metrics["automatedEqualsPackaged"],
        metrics["automatedReviewEqualsManual"], metrics["manualEqualsPackaged"],
        metrics["alphaIoU"] == 1.0, metrics["rgbMae"] == 0.0,
        metrics["alphaMae"] == 0.0, metrics["reviewRgbMae"] == 0.0,
        metrics["rgbMaxAbsError"] == 0, metrics["alphaMaxAbsError"] == 0,
        metrics["reviewRgbMaxAbsError"] == 0, metrics["routedEdgeDisplacementPx"] == 0.0,
        all(metrics["transparentRgbZero"].values()),
        all(metrics["pngFileEquality"].values()),
    )
    return {
        "accepted": bool(all(mandatory)),
        "artifacts": {"automatedRgba": "stage-1-auto-rgba.png", "automatedReview": "stage-1-auto-review.png", "comparison": "stage-1-auto-vs-manual.png", "differenceHeatmap": "stage-1-diff-heatmap.png", "repairMask": "stage-1-repair-mask.png", "review": "stage-1-review.png"},
        "copiedAcceptedPngBytes": {"automatedReview": True, "automatedRgba": True},
        "pieces": [{"id": identifier, "index": index, "material": material} for identifier, index, material in policy.pieces],
        "profile": {"id": policy.profile_id, "material": policy.material, "pieceCount": policy.piece_count, "productId": policy.product_id},
        "source": {"decodedRgbSha256": policy.source.pixel_sha256, "id": policy.source_id, "rawJpegSha256": policy.source.raw_sha256},
        "stage": 1,
        "stage0AcceptanceSha256": policy.stage0_acceptance_sha256,
        "stage0SelectedSourceId": _mapping(stage0["selectedSource"], "Stage 0 selected source")["id"],
        "validation": metrics,
    }


def _comparison_metrics(
    automated: np.ndarray, manual: _ValidatedImage, automated_review: np.ndarray,
    manual_review: _ValidatedImage, packaged: _ValidatedImage, root: Path,
) -> dict[str, object]:
    rgba, review, packaged_rgba = manual.pixels, manual_review.pixels, packaged.pixels
    rgb_error = np.abs(automated[..., :3].astype(np.int16) - rgba[..., :3].astype(np.int16))
    alpha_error = np.abs(automated[..., 3].astype(np.int16) - rgba[..., 3].astype(np.int16))
    ...
    return {
        "alphaIoU": float(np.count_nonzero(auto_alpha & manual_alpha) / union) if union else 1.0,
        "alphaMae": float(alpha_error.mean()), "alphaMaxAbsError": int(alpha_error.max()),
        ...
        "automatedReviewPixelSha256": _array_sha256(automated_review), "automatedRgbaPixelSha256": _array_sha256(automated),
        "manualEqualsPackaged": bool(np.array_equal(rgba, packaged_rgba)),
        "manualReviewPixelSha256": _array_sha256(review), "manualRgbaPixelSha256": _array_sha256(rgba),
        "packagedRgbaPixelSha256": _array_sha256(packaged_rgba),
        "pngFileEquality": {"automatedReviewEqualsManual": auto_review_bytes == manual_review.raw_bytes, "automatedRgbaEqualsManual": auto_rgba_bytes == manual.raw_bytes, "automatedRgbaEqualsPackaged": auto_rgba_bytes == packaged.raw_bytes},
        "pngFileSha256": {"automatedReview": _bytes_sha256(auto_review_bytes), "automatedRgba": _bytes_sha256(auto_rgba_bytes), "manualReview": _bytes_sha256(manual_review.raw_bytes), "manualRgba": _bytes_sha256(manual.raw_bytes), "packagedRgba": _bytes_sha256(packaged.raw_bytes)},
        "reviewRgbMae": float(review_error.mean()), "reviewRgbMaxAbsError": int(review_error.max()),
        "rgbMae": float(rgb_error.mean()), "rgbMaxAbsError": int(rgb_error.max()),
        "routedEdgeDisplacementPx": _edge_displacement(automated, rgba), "shape": list(automated.shape),
        "transparentRgbZero": {"automated": _transparent_rgb_is_zero(automated), "manual": _transparent_rgb_is_zero(rgba), "packaged": _transparent_rgb_is_zero(packaged_rgba)},
    }
...
def _edge_displacement(first: np.ndarray, second: np.ndarray) -> float:
    ...
    if np.array_equal(first_edges, second_edges): return 0.0
    def directed(source: np.ndarray, target: np.ndarray) -> float:
        if not np.any(source) or not np.any(target): return float("inf")
        return float(cv2.distanceTransform((~target).astype(np.uint8), cv2.DIST_L2, 5)[source].max())
...
def _side_by_side(automated: np.ndarray, manual: np.ndarray) -> np.ndarray:
    ...
    canvas.paste(Image.fromarray(automated), (24, 54)); canvas.paste(Image.fromarray(manual), (width + 48, 54))
...
def _difference_heatmap(automated: np.ndarray, manual: np.ndarray) -> np.ndarray:
    difference = np.max(np.abs(automated.astype(np.int16) - manual.astype(np.int16)), axis=2).astype(np.uint8)
    amplified = np.clip(difference.astype(np.uint16) * 16, 0, 255).astype(np.uint8)
    heat = cv2.cvtColor(cv2.applyColorMap(amplified, cv2.COLORMAP_TURBO), cv2.COLOR_BGR2RGB); heat[amplified == 0] = (0, 0, 0)
    canvas = Image.new("RGB", (1040, 323), _BACKGROUND); draw = ImageDraw.Draw(canvas)
    draw.text((20, 18), "Stage 1 — amplified absolute RGBA difference (16×; black means exact)", fill=(34, 34, 34), font=ImageFont.load_default())
    canvas.paste(Image.fromarray(heat), (20, 44)); return np.asarray(canvas)


def _repair_mask_image(mask: np.ndarray) -> np.ndarray:
    canvas = np.full((*mask.shape, 3), 255, dtype=np.uint8); canvas[mask > 0] = (222, 65, 52); return canvas


def _combined_review(source: np.ndarray, repair_mask: np.ndarray, automated_review: np.ndarray, manual_review: np.ndarray, heatmap: np.ndarray) -> np.ndarray:
    panels = (("Registered source", source), ("Repair mask", _repair_mask_image(repair_mask)), ("Automated cleanup", automated_review), ("Accepted manual cleanup", manual_review), ("Amplified exact-difference check", heatmap))
    cell_width, cell_height = 1240, 500; canvas = Image.new("RGB", (cell_width * 2, cell_height * 3), _BACKGROUND); draw = ImageDraw.Draw(canvas); font = ImageFont.load_default()
    for index, (title, pixels) in enumerate(panels):
        column, row = index % 2, index // 2; x, y = column * cell_width + 20, row * cell_height + 20
        image = Image.fromarray(pixels).convert("RGB"); image.thumbnail((cell_width - 40, cell_height - 64), Image.Resampling.LANCZOS)
        canvas.paste(image, (x, y + 24)); draw.text((x, y), title, fill=(34, 34, 34), font=font)
    return np.asarray(canvas)


def _write_accepted_bytes(path: Path, pixels: np.ndarray, accepted: bytes, mode: str) -> None:
    with Image.open(BytesIO(accepted)) as image:
        if image.format != "PNG" or image.mode != mode or not np.array_equal(pixels, np.asarray(image)):
...
def _bytes_sha256(value: bytes) -> str: return sha256(value).hexdigest()
def _array_sha256(value: np.ndarray) -> str: return sha256(value.tobytes()).hexdigest()
def _transparent_rgb_is_zero(rgba: np.ndarray) -> bool: return bool(np.all(rgba[..., :3][rgba[..., 3] == 0] == 0))
