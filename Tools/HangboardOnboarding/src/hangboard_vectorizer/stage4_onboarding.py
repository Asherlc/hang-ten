...
class Stage4EvidenceInputs:
    ...
    @classmethod
    def from_roots(
        cls, stage1_root: Path, stage2_root: Path, stage3_root: Path
    ) -> "Stage4EvidenceInputs":
        return cls(
            stage1_root / "stage-1-acceptance.json",
            stage1_root / "stage-1-auto-rgba.png",
            stage2_root / "stage-2-acceptance.json",
            stage3_root / "stage-3-acceptance.json",
            stage3_root / "stage-3-vector-regions.json",
            stage3_root / "stage-3-vector.svg",
        )


def run_stage4_onboarding(
    evidence: Stage4EvidenceInputs,
    artifact_root: Path,
    *,
    manual_oracle_path: Path | None = None,
    browser_oracle_path: Path | None = None,
    ...
    source_png, document, stage3_digest, stage3_acceptance = _validate_evidence(
        evidence
    )
    beastmaker_mode = oracle_paths is not None or comparison_policy == "beastmaker-v14"
    first, second = (
        compose_stage4(source_png, document),
        compose_stage4(source_png, document),
    )
    if first != second:
    ...
    try:
        ...
        raise FileExistsError(
            f"stage 4 publication is already in progress: {artifact_root}"
        ) from error
    ...
    try:
        ...
        try:
            ...
            evaluator = importlib.import_module(
                "hangboard_vectorizer.stage4_evaluation"
            )
            acceptance = evaluator.evaluate_stage4_candidate(
                temporary,
                source_png=source_png,
                stage3_document=document,
                stage3_acceptance_raw_sha256=stage3_digest,
                manual_oracle_path=manual_oracle_path,
                browser_oracle_path=browser_oracle_path,
            ...
            if artifact_root.exists():
                raise FileExistsError(
                    f"stage 4 artifact root already exists: {artifact_root}"
                )
...
def _validate_evidence(
    evidence: Stage4EvidenceInputs,
) -> tuple[bytes, dict[str, object], str, dict[str, object]]:
    ...
    if (
        evidence.stage1_rgba_path.name,
        evidence.stage3_regions_path.name,
        evidence.stage3_svg_path.name,
    ) != ("stage-1-auto-rgba.png", "stage-3-vector-regions.json", "stage-3-vector.svg"):
    ...
    stage1, stage2, stage3 = (
        _json(value, name)
        for value, name in (
            (stage1_raw, "Stage 1 acceptance"),
            (stage2_raw, "Stage 2 acceptance"),
            (stage3_raw, "Stage 3 acceptance"),
        )
    )
    if (
        stage1.get("accepted") is not True
        or stage1.get("stage") != 1
        or stage2.get("accepted") is not True
        or stage2.get("stage") != 2
        or stage3.get("accepted") is not True
        or stage3.get("stage") != 3
    ):
        raise ValueError(
            "Stage 4 requires accepted Stage 1, Stage 2, and Stage 3 evidence"
        )
    ...
    if (
        ...
        or not isinstance(validation.get("pngFileSha256"), dict)
        or validation["pngFileSha256"].get("automatedRgba")
        != sha256(source_png).hexdigest()
    ...
    if (
        ...
        or generation.get("stage1AcceptanceRawSha256") != sha256(stage1_raw).hexdigest()
    ):
    ...
    if (
        not source_entry.is_file()
        or stage2_hashes.get(source_entry.name)
        != sha256(source_entry.read_bytes()).hexdigest()
    ):
        raise ValueError("accepted Stage 2 source evidence was changed")
    if (
        generation.get("stage1PixelSha256") != source_pixel_sha
        or _rgba_pixel_sha(source_entry.read_bytes()) != source_pixel_sha
    ):
    ...
    _validate_stage3_svg(
        evidence.stage3_svg_path,
        document,
        int(document["width"]),
        int(document["height"]),
    )
...
def _validate_stage3_svg(
    path: Path, document: Mapping[str, object], width: int, height: int
) -> None:
    try:
    ...
    if (
        ...
        or len(paths) != len(regions) + len(pieces)
    ):
...
def _validate_stage3_geometry(
    document: Mapping[str, object], source_png: bytes
) -> None:
    ...
    if not isinstance(profile_value, dict) or set(profile_value) != set(
        asdict(VectorizationProfile())
    ):
    ...
    if (
        ...
        or not isinstance(regions_value, list)
        or not pieces_value
    ):
    ...
    for index, (item, raw) in enumerate(zip(pieces_value, raw_pieces, strict=True)):
        ...
        if (
            not np.any(output)
            or item.get("rawMaskSha256") != sha256(raw.tobytes()).hexdigest()
    ...
    for item in regions_value:
        if (
            not isinstance(item, dict)
            or type(item.get("pieceIndex")) is not int
            or item["pieceIndex"] not in range(len(pieces_value))
        ):
        ...
        validate_display_path(
            path, scale_basis=float(min(_bbox_size(independent))), profile=profile
        )
...
def _validate_stage3_seams(
    ...
    piece_rasters: Mapping[str, np.ndarray],
    width: int,
    height: int,
) -> None:
    ...
    for index, item in enumerate(values, start=1):
        if (
            not isinstance(item, dict)
            or item.get("id") != f"seam-{index:03d}"
            or item.get("kind") not in {"piece", "region"}
            or item.get("firstId") not in entities
            or item.get("secondId") not in entities
        ):
        ...
        if (
            raw_points[0] != seam.start
            or raw_points[-1] != seam.end
            or _ordered_seam(raw_edges) not in {raw_points, tuple(reversed(raw_points))}
        ):
    ...
    for collection in (document["pieces"], document["regions"]):
        ...
        for item in collection:
            ...
            if (
                ...
                or set(shared) != memberships[item["id"]]
            ):
                raise ValueError("accepted Stage 3 shared seam membership is invalid")
    expected_pairs = _expected_stage3_seam_pairs(
        document, regions, piece_rasters, width, height
    )
...
def _expected_stage3_seam_pairs(
    ...
    surfaces = [
        ...
        if isinstance(item, dict) and item.get("type") != "pocket"
    ]
    rasters = rasterize_paths_independent(
        tuple(regions[item["id"]] for item in surfaces), width, height, scale=scale
    )
    boundaries = {
        item["id"]: _boundary(mask)
        for item, mask in zip(surfaces, rasters, strict=True)
    }
    upscaled_pieces = {
        owner: cv2.resize(
            raster,
        ...
        for owner, raster in piece_rasters.items()
    }
...
def _write_candidate(root: Path, candidate: Stage4Candidate) -> None:
    ...
    _write_json(
        root / "stage-4-candidate-hashes.json",
        {
            "files": {
                name: sha256(values[name]).hexdigest() for name in _CANDIDATE_NAMES
            },
            "schemaVersion": 1,
        },
    )
