...
from .hold_refinement import (
    HoldDiscoveryProfile,
    assign_aperture_candidates,
...
from .semantic_holds import (
    ...
    validate_semantic_grip_response,
)
...
def _freeze(
    mask: np.ndarray, dtype: np.dtype | type[np.generic] = np.uint8
) -> np.ndarray:
...
class HoldDiscovery:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "candidate_evidence_rgb", _freeze(self.candidate_evidence_rgb)
        )
...
def discover_holds(
    ...
    *,
    ...
    anchors = {
        hint.hint_key: _anchor_point(hint, width, height, profile) for hint in hints
    }
    ...
    for piece_index, piece_mask in enumerate(piece_masks):
        apertures = tuple(
            hint
            for hint in hints
            if hint.piece_index == piece_index and hint.visual_mode == "aperture"
        )
        surfaces = tuple(
            hint
            for hint in hints
            if hint.piece_index == piece_index and hint.visual_mode == "surface"
        )
        ...
        for hint in apertures:
            value = refine_aperture(
                ...
                exclusion_masks=tuple(value.mask for value in surface_refined.values()),
                gray=gray,
                profile=profile,
            )
    ...
    return HoldDiscovery(
        tuple(holds), labels, evidence_rgb, response, request.pixel_sha256
    )
...
def _anchor_point(
    hint: CoarseHoldHint, width: int, height: int, profile: HoldDiscoveryProfile
) -> tuple[float, float]:
    return (
        (hint.anchor[0] + 0.5) * width / profile.grid_width,
        (hint.anchor[1] + 0.5) * height / profile.grid_height,
    )
...
def _piece_masks(alpha: np.ndarray, piece_count: int) -> tuple[np.ndarray, ...]:
    ...
    components = [
        ...
        if stats[index, cv2.CC_STAT_AREA] >= max(16, binary.size // 1000)
    ]
...
def _stable_hint_order(
    hints: tuple[CoarseHoldHint, ...],
) -> tuple[tuple[int, CoarseHoldHint], ...]:
    ...
    for hint in sorted(
        ...
        if (
            not rows
            or rows[-1][0].piece_index != hint.piece_index
            or hint.anchor[1] != rows[-1][0].anchor[1]
        ):
            rows.append([hint])
        else:
            rows[-1].append(hint)
    ...
    for row in rows:
        ...
        ordered.extend(
            (piece_rows[piece], hint)
            for hint in sorted(row, key=lambda value: (value.anchor[0], value.hint_key))
        )
...
def _mask_geometry(
    mask: np.ndarray,
) -> tuple[tuple[Point, ...], tuple[float, float], tuple[int, int, int, int], int]:
    if not np.any(mask):
    ...
    return (
        tuple((int(px), int(py)) for px, py in contour),
        center,
        (x, y, width, height),
...
def _mask_rle(mask: np.ndarray) -> list[int]:
    flat = mask.astype(np.uint8).ravel() != 0
    if flat.size == 0:
        return [0]
...
def _candidate_evidence(
    contrast: np.ndarray, gray: np.ndarray, alpha: np.ndarray
) -> np.ndarray:
...
def discovery_identity(discovery: HoldDiscovery) -> str:
    ...
    return sha256(
        payload
        + discovery.label_raster.tobytes()
        + discovery.candidate_evidence_rgb.tobytes()
    ).hexdigest()
