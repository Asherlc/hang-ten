...
def _external_sliding_loop_cord_rig(
    ...
    if _string(payload["type"], f"{source}.type") != "externalSlidingLoop":
        raise ValueError(f"{source}.type is unsupported")
    style_payload = _mapping(payload["style"], f"{source}.style")
    _closed(
        style_payload,
    ...
    body_contact_frame = CordRect.from_json(
        payload["bodyContactFrame"], f"{source}.bodyContactFrame"
    )
    corner_radius = _number(payload["cornerRadius"], f"{source}.cornerRadius")
    if corner_radius < 0:
        raise ValueError(f"{source}.cornerRadius must be non-negative")
    if corner_radius > min(body_contact_frame.width, body_contact_frame.height) / 2:
        raise ValueError(
            f"{source}.cornerRadius must not exceed half the shorter bodyContactFrame side"
        )
    clearance = _number(payload["clearance"], f"{source}.clearance")
    if clearance < 0:
        raise ValueError(f"{source}.clearance must be non-negative")
...
def _cord_rig_from_json(value: Any, source: str) -> CordRig:
    ...
    if rig_type == "externalSlidingLoop":
        return _external_sliding_loop_cord_rig(payload, source)
    raise ValueError(f"{source}.type is unsupported")
...
def _validate_alias_presentations(
    ...
    for presentation in presentations:
        ...
        if presentation.source_presentation_id is None:
            ...
            elif isinstance(presentation.cord_rig, ExternalSlidingLoopCordRig):
        ...
        elif isinstance(source.cord_rig, ExternalSlidingLoopCordRig):
            _validate_external_sliding_loop_presentation(
                source.cord_rig,
                presentation_id=presentation.id,
                rotation_degrees=rotation_degrees,
