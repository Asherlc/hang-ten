...
final class CheckoutSelection {
    enum ValidationError: LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            "Choose a Hang Ten checkout containing .git, Tools/HangboardOnboarding/boards, and Tools/hold-highlight-editor."
        }
    }
    ...
    private static func validatedURL(_ url: URL, fileManager: FileManager) throws -> URL {
        ...
        guard isDirectory(normalized, fileManager: fileManager),
              ...
              isDirectory(
                  normalized.appending(path: "Tools/HangboardOnboarding/boards"),
                  fileManager: fileManager
              ),
              isDirectory(
                  normalized.appending(path: "Tools/hold-highlight-editor"),
                  fileManager: fileManager
              ) else {
            throw ValidationError.notHangTenCheckout
        }
