...
def compile_reusable_descriptor(
    ...
    return ModelDescriptorV2._from_validated(
        ...
        _compile_contact_slots(bindings, vertices, raw_bounds, slots),
    )
...
def compile_descriptor(
    ...
    contacts = _compile_contacts(bindings, vertices, raw_model_bounds)
...
def _compile_contact_slots(
    ...
    contacts = _compile_contacts(converted, vertices_by_node_id, model_bounds)
    return {
        slot_id: ContactSlotDescriptorV2._from_validated(
            contacts[slot_id].node_ids, contacts[slot_id].face_plane_aabb, contacts[slot_id].center
        )
...
def _parse_contacts(value: object, nodes: Sequence[NodeBinding]) -> dict[str, ContactDescriptorV1]:
    ...
    for contact_id, raw_contact in contacts.items():
        ...
        parsed[contact_id] = ContactDescriptorV1._from_validated(
            tuple(node_ids), face_bounds, center
        )
    return parsed
...
def _parse_contact_slots(value: object, nodes: Sequence[SlotNodeBinding]) -> dict[str, ContactSlotDescriptorV2]:
    ...
    for slot_id, raw_slot in slots.items():
        ...
        _require_exact_keys(mapping, frozenset({"nodeIDs", "facePlaneAABB", "center"}), f"contact slot {slot_id}")
        ...
        parsed[slot_id] = ContactSlotDescriptorV2._from_validated(tuple(node_ids), face_bounds, center)
    return parsed
