...
- `model` has a `.usdz` `assetPath`, a generated `.model.json` `descriptorPath`,
  ...
  views of the same face). Selection disambiguation: when a hold appears in
  multiple positions, the runtime selector chooses the position whose rotation
  is closest to the current view quaternion (minimum geodesic distance on the
  unit quaternion sphere); if distances are equal or no orientation metadata is
  present, the first position in authored array order containing the hold is
  selected.
