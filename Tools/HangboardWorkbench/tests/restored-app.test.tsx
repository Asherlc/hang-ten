...
test("the React shell preserves the direct-workbench DOM and renders logged-out auth safely", async () => {
  await withApp(dependenciesFixture({
    ...
    assert.equal(app.text("#board-status"), "Choose a board to edit its contacts.");
    ...
    assert.equal(app.text("#empty-state"), "Select a boardIts image and contacts load together.");
    assert.equal(app.text("#contact-heading"), "No selection");
    assert.equal(app.text("#contact-empty"), "Select a contact to edit its facts and contour.");
    const requiredIds = [
      ...
      "board-name", "add-contact-button", "canvas-viewport", "editor-svg", "board-image", "contact-overlay",
      "empty-state", "validation-panel", "validation-list", "editor-status", "contact-heading", "contact-empty",
      "contact-form", "contact-id", "contact-kind-select", "contact-rotate-ccw-button", "contact-rotate-cw-button",
      "contact-rotate-input", "contact-rotate-apply-button", "delete-contact-button",
    ];
...
test("the board list shows an accessible attention status only for flagged boards", async () => {
  await withApp(dependenciesFixture({
    ...
    assert.equal(app.text("#board-list button:first-child"), "Board A1 contactsNeeds attention");
    assert.equal(app.text("#board-list button:nth-child(2)"), "Board B1 contacts");
  });
});
...
test("the board list marks model-only packages read-only and cannot open them", async () => {
  ...
  await withApp(dependenciesFixture({
    ...
    assert.equal(
      ...
      "Model Only19 contacts3D model editing is not supported",
    );
...
test("mobile canvas controls open the board drawer, repository sheet, and contact inspector explicitly", async () => {
  const image = imageFixture();
  const promptResponses = ["contact-2", "Contact 2", "primary", "edge", "", ""];
  let saves = 0;
  await withApp(dependenciesFixture({
    runtime: image.runtime,
    dialogs: { prompt() { return promptResponses.shift() ?? null; } },
    client: {
    ...
    for (const id of [
      ...
      "mobile-zoom-out-button", "mobile-zoom-in-button", "mobile-add-contact-button", "mobile-open-contact-sheet-button",
    ...
    assert.equal(app.disabled("#mobile-open-contact-sheet-button"), true);
    await app.click("#contact-overlay path");
    assert.equal(app.document.querySelector(".inspector-panel")?.classList.contains("mobile-sheet-open"), false);
    assert.equal(app.disabled("#mobile-open-contact-sheet-button"), false);
    await app.click("#mobile-open-contact-sheet-button");
    ...
    await app.click("#mobile-add-contact-button");
    assert.equal(app.document.querySelectorAll("#contact-overlay path").length, 2);
...
test("collapsing the mobile contact sheet retains the selected contact", async () => {
  const image = imageFixture();
  await withApp(dependenciesFixture({ runtime: image.runtime }), async (app) => {
    ...
    assert.equal(app.document.querySelector(".inspector-panel")?.classList.contains("mobile-sheet-open"), false);
    assert.equal(app.text("#contact-heading"), "Contact 1");

    await app.click("#mobile-open-contact-sheet-button");
    assert.equal(app.document.querySelector(".inspector-panel")?.classList.contains("mobile-sheet-open"), true);

    await app.click(".mobile-sheet-collapse");
...
test("selecting a contact on another board does not reopen an explicitly opened mobile sheet", async () => {
  const image = imageFixture();
  await withApp(dependenciesFixture({
    runtime: image.runtime,
    client: {
      async listBoards() {
        return [
          { boardId: "board-a", displayName: "Board A", contactCount: 1, imageUrl: "/api/boards/board-a/image", needsAttention: false },
          { boardId: "board-b", displayName: "Board B", contactCount: 1, imageUrl: "/api/boards/board-b/image", needsAttention: false },
        ];
      },
...
test("board selection locks board and Git actions and commits image plus document together", async () => {
  ...
  await withApp(dependenciesFixture({
    ...
    assert.equal(app.document.querySelectorAll("#contact-overlay path").length, 1);
  });
});
...
test("recoverable save authentication failure keeps edits and offers safe separate-tab login", async () => {
  ...
  await withApp(dependenciesFixture({
    ...
    assert.equal(saveAttempts[0]?.contacts[0]?.kind, "pinch");
    assert.equal(app.documentValue("#contact-kind-select"), "pinch");
    assert.equal(app.text("#save-state"), "Unsaved changes");
    assert.equal(app.document.querySelector("#validation-panel")?.classList.contains("hidden"), true);
...
test("turning autosave off cancels pending work and leaves later edits for manual save", async () => {
  ...
  await withApp(dependenciesFixture({
    ...
    await app.change("#contact-kind-select", "sloper");
    await app.flush(() => wait(800));
    assert.equal(saves, 0);
    assert.equal(app.text("#save-state"), "Unsaved changes");
  });
...
test("contact editing remains available during a save and survives its stale response", async () => {
  const image = imageFixture();
  const promptResponses = ["contact-2", "Contact 2", "primary", "edge", "", ""];
  ...
  await withApp(dependenciesFixture({
    ...
    assert.equal(savedDocuments.length, 1);
    assert.equal(savedDocuments[0]?.contacts[0]?.kind, "sloper");
    ...
    assert.equal(savedDocuments[1]?.contacts[0]?.kind, "pinch");
