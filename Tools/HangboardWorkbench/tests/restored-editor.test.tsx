...
import type { ContactEditorActions } from "../src/useContactEditor.ts";
import type {
  AuthStatus,
  Board,
  BoardSummary,
...
function dependenciesFixture(board = boardFixture(), options: {
  ...
  const dialogs: Dialogs = {
    ...
    prompt(message, defaultValue) {
      if (options.prompt) return options.prompt(message, defaultValue);
      if (/contact ID|ID for the mirrored contact/u.test(message)) return `new-contact-${nextContactNumber++}`;
      if (/contact name|name for the mirrored contact/u.test(message)) return "New contact";
      if (/equipment object ID/u.test(message)) return "primary";
      if (/factual kind/u.test(message)) return "jug";
      if (/comma-separated/u.test(message)) return "";
      return null;
    },
  };
...
function paths(app: ReactHarness): string[] {
  return [...app.document.querySelectorAll<SVGPathElement>("#contact-overlay .region-shape")]
...
test("multi-object boards assign new contacts and allow deliberate object reassignment", async () => {
  ...
  await withEditor(async (app) => {
    ...
    assert.equal(app.documentValue("#contact-equipment-object-input"), "left-ring");

    await app.change("#contact-equipment-object-input", "right-ring");
    await app.click("#save-button");
    await app.flush();

    const added = client.saveCalls[0]?.document.contacts.find((contact) => contact.id === "new-contact");
    assert.equal(added?.equipmentObjectID, "right-ring");
  }, dependenciesFixture(boardFixture(document), { client, prompt: () => answers.shift() ?? null }));
});

test("every raster region must resolve to one factual contact", () => {
  const document = documentFixture();
  document.regions[0]!.metadata.contactID = "missing-contact";
  assert.throws(() => controller.validateEditorDocument(document), /valid media regions/i);
});
...
test("editing a contact fact updates its single owner without duplicating it across media pieces", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.change("#contact-finger-capacity-select", "2");
    assert.equal(app.documentValue("#contact-finger-capacity-select"), "2");
    assert.equal(app.document.querySelectorAll('[data-contact-key^="a-piece-"]').length, 2);
  }, dependenciesFixture(boardFixture(document)));
});

test("switching presentations changes the focused canvas and scopes new contacts", async () => {
  const presentations = [
    {
  ...
  await withEditor(async (app) => {
    ...
    assert.equal(app.disabled("#add-contact-button"), false);
    await app.click("#add-contact-button");
...
test("surface controls are disabled while a board save is in flight", async () => {
  ...
  const client = {
    ...clientFixture([board]),
    saveBoard(_boardID: string, document: EditorDocument): Promise<Board> {
...
test("deleting a surface refreshes its library attention summary", async () => {
  ...
  const client = {
    ...clientFixture([front]),
    async listBoards(): Promise<BoardSummary[]> {
      return [{ boardId: "board-a", displayName: "Board A", contactCount: 0, imageUrl: front.imageUrl, needsAttention: true }];
    },
...
test("wheel zoom uses callbacks from the latest render", async () => {
  const inertEditor = { cancelActiveEdit: () => false } as ContactEditorActions;
...
test("contact paths expose button semantics and support Enter and Space selection", async () => {
  await withEditor(async (app) => {
    const target = app.document.querySelector<SVGPathElement>('[data-contact-key="b-piece-0"]');
    assert.equal(target?.getAttribute("role"), "button");
    assert.equal(target?.getAttribute("tabindex"), "0");
    assert.equal(target?.getAttribute("aria-label"), "Select contact b-piece-0");

    assert.equal(await app.keyDown('[data-contact-key="b-piece-0"]', "Enter"), false);
    assert.equal(app.text("#contact-heading"), "b");
    assert.equal(await app.keyDown('[data-contact-key="a-piece-0"]', " "), true);
    assert.equal(app.text("#contact-heading"), "a");
  });
});

test("modifier selection toggles highlighted contacts while plain selection replaces the batch and keeps one primary overlay", async () => {
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    ...
    assert.equal(await app.keyDown('[data-contact-key="a-piece-1"]', "Enter", { metaKey: true }), false);
    assert.equal(app.document.querySelector('[data-contact-key="a-piece-1"]')?.getAttribute("aria-pressed"), "true");
    assert.equal(app.text("#contact-heading"), "a");

    await app.click('[data-contact-key="a-piece-0"]');
    assert.equal(app.document.querySelector('[data-contact-key="a-piece-0"]')?.getAttribute("aria-pressed"), "true");
    assert.equal(app.document.querySelector('[data-contact-key="a-piece-1"]')?.getAttribute("aria-pressed"), "false");
    assert.equal(app.document.querySelector('[data-contact-key="b-piece-0"]')?.getAttribute("aria-pressed"), "false");
    assert.equal(app.text("#contact-heading"), "a");
  });
});
...
test("Command-click selection survives its trailing plain click", async () => {
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    await app.pointer('[data-contact-key="b-piece-0"]', "pointerdown", { pointerId: 42, metaKey: true });
    await app.pointer('[data-contact-key="b-piece-0"]', "pointerup", { pointerId: 42, metaKey: true });
    await app.mouse('[data-contact-key="b-piece-0"]', "click");
...
test("a cancelled Command-click does not suppress the next normal click", async () => {
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    await app.pointer('[data-contact-key="b-piece-0"]', "pointerdown", { pointerId: 42, metaKey: true });
    await app.pointer('[data-contact-key="b-piece-0"]', "pointercancel", { pointerId: 42, metaKey: true });
    await app.click('[data-contact-key="b-piece-0"]');

    assert.equal(app.document.querySelector('[data-contact-key="a-piece-0"]')?.getAttribute("aria-pressed"), "false");
    assert.equal(app.document.querySelector('[data-contact-key="b-piece-0"]')?.getAttribute("aria-pressed"), "true");
  });
});
...
test("batch deletion names every selected physical contact and leaves the document intact when cancelled", async () => {
  ...
  await withEditor(async (app) => {
    ...
    assert.deepEqual(prompts, ["Delete 2 selected contacts and all of their pieces?"]);
...
test("the contact kind control rejects values outside the v3 factual vocabulary", async () => {
  const board = boardFixture();
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');

    assert.deepEqual(
      [...app.document.querySelectorAll<HTMLOptionElement>("#contact-kind-select option")].map((option) => option.value),
      ["jug", "sloper", "edge", "pocket", "pinch", "gaston"],
    );
    await app.change("#contact-kind-select", "legacy-grip");
    assert.equal(app.documentValue("#contact-kind-select"), "jug");
    assert.match(app.text("#validation-list"), /valid factual contacts/i);
  }, dependenciesFixture(board));
});

test("changing exactly two selected physical contacts to gastons creates a reciprocal pair that saves", async () => {
  const client = clientFixture([boardFixture()]);
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    await app.mouse('[data-contact-key="b-piece-0"]', "click", { ctrlKey: true });

    await app.change("#contact-kind-select", "gaston");
    await app.click("#save-button");

    assert.equal(client.saveCalls.length, 1);
    assert.deepEqual(client.saveCalls[0]?.document.contacts.map((contact) => [contact.kind, contact.pairedContactID]), [
      ["gaston", "b"],
      ["gaston", "a"],
    ]);
...
test("converting a selected gaston would orphan its unselected counterpart, so it leaves the pair unchanged", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.mouse('[data-contact-key="right-piece-0"]', "click", { ctrlKey: true });
...
test("converting two pieces of one contact to gastons leaves the fact unchanged with validation", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.mouse('[data-contact-key="same-b-piece-0"]', "click", { ctrlKey: true });

    await app.change("#contact-kind-select", "gaston");

    assert.equal(app.documentValue("#contact-kind-select"), "jug");
    assert.match(app.text("#validation-list"), /two distinct contact IDs/i);

    await app.click("#save-button");
    assert.equal(client.saveCalls.length, 1);
    assert.deepEqual(client.saveCalls[0]?.document.contacts.map((contact) => contact.kind), ["jug"]);
  }, dependenciesFixture(board, { client }));
});
...
test("an unpaired gaston offers only actionable pairing candidates and saves the initial reciprocal pair", async () => {
  ...
  await withEditor(async (app) => {
    ...
    assert.equal(app.documentValue("#contact-kind-select"), "gaston");
    assert.equal(app.text('#contact-gaston-pair-select option[value=""]'), "Choose a contact");
    assert.equal(app.document.querySelector('#contact-gaston-pair-select option[value="left"]'), null);
    assert.equal(app.text('#contact-gaston-pair-select option[value="right"]'), "right");
    assert.equal(app.document.querySelector('#contact-gaston-pair-select option[value="jug"]'), null);
    assert.equal(app.document.querySelector('#contact-gaston-pair-select option[value="other-a"]'), null);
    assert.equal(app.document.querySelector('#contact-gaston-pair-select option[value="other-b"]'), null);

    await app.change("#contact-gaston-pair-select", "right");
    await app.click("#save-button");

    assert.equal(client.saveCalls[0]?.document.contacts[0]?.pairedContactID, "right");
    assert.equal(client.saveCalls[0]?.document.contacts[1]?.pairedContactID, "left");
  }, dependenciesFixture(board, { client }));
});
...
test("saving an unpaired gaston stays local and explains that it needs a pair", async () => {
  ...
  await withEditor(async (app) => {
    ...
    assert.match(app.text("#validation-list"), /contact left needs a paired gaston contact/i);
  }, dependenciesFixture(board, { client }));
});
...
test("Add contact creates factual ownership and selects a centered square region", async () => {
  await withEditor(async (app) => {
    await app.click("#add-contact-button");
...
test("Add segment creates and selects a separately editable piece of the selected contact", async () => {
  const board = boardFixture();
  ...
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    await app.click("#add-contact-piece-button");

    assert.equal(app.text("#contact-heading"), "a");
    assert.equal(app.document.querySelector('[data-contact-key="a-piece-0"]')?.getAttribute("aria-pressed"), "false");
    assert.equal(app.document.querySelector('[data-contact-key="a-piece-2"]')?.getAttribute("aria-pressed"), "true");
    ...
    assert.deepEqual(saved[0]?.regions.at(-1), {
      ...
      metadata: { contactID: "a", pieceIndex: 2, presentationID: "primary" },
    });
  }, dependenciesFixture(board, { client }));
});

test("delete and type changes apply to every piece sharing contactID", async () => {
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    await app.change("#contact-kind-select", "pinch");
...
test("changing a sloper contact kind preserves independently-owned raster treatment", async () => {
  ...
  await withEditor(async (app) => {
    ...
    assert.equal(saved[0]?.contacts.find((contact) => contact.id === "a")?.kind, "pinch");
    assert.deepEqual(saved[0]?.regions.map((region) => region.treatment), [{ type: "surface" }, { type: "surface" }]);
  }, dependenciesFixture(board, { client }));
});
...
test("duplicate and mirror reflects every selected physical contact with fresh contact identities", async () => {
  ...
  await withEditor(async (app) => {
    ...
    assert.equal(app.text("#contact-heading"), "New contact");
    assert.equal(app.document.querySelector('[data-contact-key="a-piece-0"]')?.getAttribute("aria-pressed"), "false");
    assert.equal(app.document.querySelector('[data-contact-key="new-contact-1-piece-0"]')?.getAttribute("aria-pressed"), "true");
    assert.equal(app.document.querySelector('[data-contact-key="new-contact-1-piece-1"]')?.getAttribute("aria-pressed"), "true");
    ...
    assert.deepEqual(saved[0]?.regions.slice(3), [
      {
        id: 4,
        key: "new-contact-1-piece-0",
        displayPath: "M 90 10 Q 85 5 80 10 L 80 20 Z",
        metadata: { contactID: "new-contact-1", pieceIndex: 0, presentationID: "primary" },
        shapeConstraint: { shape: "oval", rotationDegrees: -15 },
      },
      {
        id: 5,
        key: "new-contact-1-piece-1",
        displayPath: "M 70 10 L 60 10 L 60 20 Z",
        metadata: { contactID: "new-contact-1", pieceIndex: 1, presentationID: "primary" },
      },
    ]);
  }, dependenciesFixture(board, { client }));
});
...
test("duplicate and mirror regenerates bendable indexes from mirrored cubic commands", async () => {
  ...
  await withEditor(async (app) => {
    await app.click('[data-contact-key="curve-piece-0"]');
    await app.click("#duplicate-mirror-contact-button");
    await app.click("#save-button");
    await app.flush();

    const duplicate = saved[0]?.regions.find((region) => region.metadata?.contactID !== "curve");
...
test("contact depth ranges load, update atomically, and save on factual contacts", async () => {
  ...
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    assert.equal(app.documentValue("#contact-depth-lower-input"), "7.5");
...
test("equal contact depth bounds reopen and can become a range or be cleared", async () => {
  ...
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    assert.equal(app.documentValue("#contact-depth-lower-input"), "10");
    assert.equal(app.documentValue("#contact-depth-upper-input"), "10");
...
test("changing the selected contact replaces the visible factual depth range", async () => {
  ...
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    await app.change("#contact-depth-lower-input", "9");
    await app.click('[data-contact-key="b-piece-0"]');
...
test("a new edit clears redo and a completed drag is a single undo step", async () => {
  await withEditor(async (app) => {
    ...
    await app.click('[data-contact-key="a-piece-0"]');
    await drag(app, '[data-contact-key="a-piece-0"]', [{ x: 15, y: 15 }, { x: 20, y: 15 }, { x: 25, y: 15 }]);
    assert.equal(await app.keyDown("body", "z", { ctrlKey: true }), true);
    assert.equal(paths(app)[0], FIRST_PATH);
    await app.keyDown("body", "ArrowRight");
    assert.equal(await app.keyDown("body", "y", { ctrlKey: true }), false);
...
test("undo during a drag restores the committed document before building redo history", async () => {
  await withEditor(async (app) => {
    ...
    await app.pointer('[data-contact-key="a-piece-0"]', "pointerdown", { pointerId: 7, clientX: 16, clientY: 15 });
...
test("command/control undo and Ctrl redo consume browser shortcuts when cancelling an active drag without history", async () => {
  for (const shortcut of [
    ...
    await withEditor(async (app) => {
      app.setSvgGeometry("#editor-svg", { rect: { left: 0, top: 0, width: 100, height: 50 } });
      await app.click('[data-contact-key="a-piece-0"]');
      await app.pointer('[data-contact-key="a-piece-0"]', "pointerdown", { pointerId: 7, clientX: 15, clientY: 15 });
...
test("no-op drags do not create an undo revision", async () => {
  await withEditor(async (app) => {
    app.setSvgGeometry("#editor-svg", { rect: { left: 0, top: 0, width: 100, height: 50 } });
    await app.click('[data-contact-key="a-piece-0"]');
    await drag(app, '[data-contact-key="a-piece-0"]', [{ x: 15, y: 15 }]);
    assert.equal(app.text("#save-state"), "Saved");
    assert.equal(await app.keyDown("body", "z", { ctrlKey: true }), false);
    assert.equal(paths(app)[0], FIRST_PATH);

    await drag(app, '[data-contact-key="a-piece-0"]', [{ x: 15, y: 15 }, { x: 25, y: 15 }, { x: 15, y: 15 }]);
    assert.equal(paths(app)[0], FIRST_PATH);
    assert.equal(app.text("#save-state"), "Saved");
    assert.equal(await app.keyDown("body", "z", { ctrlKey: true }), false);
  });
...
test("bracket keys and buttons rotate by 15 and 45 degrees", async () => {
  ...
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    await app.mouse("#contact-rotate-cw-button", "click");
    assert.equal(paths(app)[0], rotate(FIRST_PATH, 15, pivot));
  });
  await withEditor(async (app) => {
    await app.click('[data-contact-key="a-piece-0"]');
    await app.mouse("#contact-rotate-ccw-button", "click", { shiftKey: true });
    assert.equal(paths(app)[0], rotate(FIRST_PATH, -45, pivot));
  });
});
...
test("saving a stable editable path sends no local geometry IDs", async () => {
  ...
  await withEditor(async (app) => {
    app.setSvgGeometry("#editor-svg", { rect: { left: 0, top: 0, width: 100, height: 50 } });
    await app.click('[data-contact-key="stable-piece-0"]');
...
test("whole-path dragging snaps its horizontal and vertical bounds edges to nearby guides", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.click("#add-horizontal-guide-button");
    await app.click("#add-vertical-guide-button");
    await app.click('[data-contact-key="snap-target"]');
    await drag(app, '[data-contact-key="snap-target"]', [{ x: 30, y: 30 }, { x: 14, y: 14 }]);
...
test("whole-path dragging does not snap when only its center is near a guide", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.click("#add-vertical-guide-button");
    await app.click('[data-contact-key="snap-target"]');
    await drag(app, '[data-contact-key="snap-target"]', [{ x: 30, y: 30 }, { x: 13, y: 13 }]);
...
test("Alt bypasses guide snapping during a whole-path drag", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.click("#add-vertical-guide-button");
    await app.click('[data-contact-key="snap-target"]');
    await app.pointer('[data-contact-key="snap-target"]', "pointerdown", { pointerId: 7, clientX: 30, clientY: 30 });
...
test("vertex, control, and whole-path drags derive every move from pointer-down geometry", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await drag(app, '[data-contact-key="curve"]', [{ x: 10, y: 10 }, { x: 12, y: 12 }, { x: 15, y: 15 }]);
...
test("rotation drag rotates every Command-selected physical contact around its own centroid", async () => {
  await withEditor(async (app) => {
    app.setSvgGeometry("#editor-svg", { rect: { left: 0, top: 0, width: 100, height: 50 } });
    await app.click('[data-contact-key="a-piece-0"]');
    await app.mouse('[data-contact-key="b-piece-0"]', "click", { ctrlKey: true });
    ...
    const physicalAPivot = contactCentroid(documentFixture().regions.slice(0, 2), pathEditor);
    const physicalBPivot = contactCentroid([documentFixture().regions[2]!], pathEditor);
...
test("straight-segment menu converts a segment to a bendable curve with controls at both endpoints", async () => {
  ...
  await withEditor(async (app) => {
    app.setSvgGeometry("#editor-svg", { rect: { left: 0, top: 0, width: 100, height: 50 } });
    await app.click('[data-contact-key="square"]');
    await app.mouse('[data-contact-key="square"]', "contextmenu", { button: 2, clientX: 20, clientY: 10 });
    ...
    const invokingLine = app.document.querySelector<SVGPathElement>('[data-contact-key="square"]');
...
test("dragging a cubic made bendable by its menu action pulls its midpoint to the pointer", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await drag(app, '[data-contact-key="square"]', [{ x: 20, y: 10 }, { x: 20, y: 20 }]);
...
test("splitting a marked cubic leaves either descendant pullable", async () => {
  ...
  for (const { start, end, commandIndex, expectedControl } of [
    ...
    await withEditor(async (app) => {
      ...
      await app.click('[data-contact-key="curve"]');
      await app.mouse("#editor-svg", "dblclick", { clientX: 20, clientY: 10 });
      await drag(app, '[data-contact-key="curve"]', [start, end]);
...
test("dragging a constrained cubic moves its whole path instead of bending it", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await drag(app, '[data-contact-key="oval"]', [{ x: 20, y: 10 }, { x: 20, y: 20 }]);
...
test("dragging an imported custom cubic moves its whole path instead of bending it", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await drag(app, '[data-contact-key="custom"]', [{ x: 20, y: 10 }, { x: 20, y: 20 }]);
...
test("controls stay uniquely addressable after converting two segments to bendable curves", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.mouse('[data-contact-key="square"]', "contextmenu", { button: 2, clientX: 20, clientY: 10 });
    await app.click("#make-bendable-action");
    await app.mouse('[data-contact-key="square"]', "contextmenu", { button: 2, clientX: 30, clientY: 20 });
    await app.click("#make-bendable-action");
...
test("line menu snaps a diagonal custom-outline segment to the chosen axis", async () => {
  for (const [action, expectedPath, expectedStatus] of [
    ...
    await withEditor(async (app) => {
      ...
      await app.mouse('[data-contact-key="diagonal"]', "contextmenu", { button: 2, clientX: 20, clientY: 15 });
...
test("curved-segment menu makes a quadratic segment straight and removes its control", async () => {
  ...
  await withEditor(async (app) => {
    app.setSvgGeometry("#editor-svg", { rect: { left: 0, top: 0, width: 100, height: 50 } });
    await app.click('[data-contact-key="square"]');
    await app.mouse('[data-contact-key="square"]', "contextmenu", { button: 2, clientX: 20, clientY: 5 });
...
test("curved-segment menu presents adding a smooth point at the right-click location", async () => {
  ...
  await withEditor(async (app) => {
    app.setSvgGeometry("#editor-svg", { rect: { left: 0, top: 0, width: 100, height: 50 } });
    await app.click('[data-contact-key="square"]');
    await app.mouse('[data-contact-key="square"]', "contextmenu", { button: 2, clientX: 12.5, clientY: 27.5 });
...
test("a serialized and dragged quadratic inflection point remains removable", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.click('[data-contact-key="square"]');
    await app.mouse('[data-contact-key="square"]', "contextmenu", { button: 2, clientX: 6.456789, clientY: 17.654321 });
    await app.click("#add-inflection-point-action");
...
test("straight-segment context menu chooses the closest eligible edge", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.mouse('[data-contact-key="square"]', "contextmenu", { button: 2, clientX: 28, clientY: 15 });
    await app.click("#make-bendable-action");
...
test("changing contacts closes a stale line menu before it can edit the new selection", async () => {
  ...
  await withEditor(async (app) => {
    ...
    await app.mouse('[data-contact-key="first"]', "contextmenu", { button: 2, clientX: 20, clientY: 10 });
    assert.ok(app.document.querySelector('[role="menu"]'));

    await app.click('[data-contact-key="second"]');
...
test("Delete and Backspace remove only the selected eligible vertex and ignore form targets", async () => {
  ...
  await withEditor(async (app) => {
    ...
    for (const selector of ["#contact-rotate-input", "#contact-kind-select"]) {
    ...
    const editable = app.document.querySelector<HTMLElement>("#contact-heading");
    assert.ok(editable);
    editable.setAttribute("contenteditable", "true");
    await app.flush(() => editable.focus());
    assert.equal(await app.keyDown("#contact-heading", "Delete"), false);
    assert.equal(paths(app)[0], "M 10 10 L 20 10 L 30 10 L 30 30 L 10 30 Z");
...
test("malformed selected paths report interaction failures instead of throwing", async () => {
  ...
  await withEditor(async (app) => {
    app.setSvgGeometry("#editor-svg", { rect: { left: 0, top: 0, width: 100, height: 50 } });
    await app.click('[data-contact-key="malformed-piece-0"]');
    await app.pointer('[data-contact-key="malformed-piece-0"]', "pointerdown", {
      pointerId: 7,
      clientX: 10,
      clientY: 10,
    });
...
test("outline picker reflects persisted constraints and changes every piece of the selected physical contact", async () => {
  ...
  await withEditor(async (app) => {
    ...
    assert.deepEqual(
      [...app.document.querySelectorAll<HTMLOptionElement>("#contact-outline-shape-select option")]
        .map((option) => [option.value, option.textContent]),
      [["custom", "Custom"], ["oval", "Oval"], ["circle", "Circle"], ["pill", "Pill"], ["roundedRectangle", "Rounded rectangle"], ["rectangle", "Rectangle"]],
    );
    await app.change("#contact-outline-shape-select", "oval");
    assert.deepEqual(paths(app), [
    ...
    await app.click('[data-contact-key="a-piece-1"]');
    assert.equal(app.documentValue("#contact-outline-shape-select"), "oval");
  }, dependenciesFixture(board));
});
...
test("invalid outline actions preserve geometry while save-in-progress edits remain unsaved", async () => {
  ...
  await withEditor(async (app) => {
    ...
    const originalPath = paths(app)[0];
    await app.click("#save-button");
    assert.equal(app.disabled("#contact-outline-shape-select"), false);
    assert.equal(app.disabled("#contact-kind-select"), false);
    assert.equal(app.disabled("#contact-rotate-cw-button"), false);
    assert.equal(app.disabled("#delete-contact-button"), false);
    await app.change("#contact-outline-shape-select", "oval");
  ...
  await withEditor(async (app) => {
    ...
    await app.change("#contact-outline-shape-select", "oval");
    assert.equal(paths(app)[0], originalPath);
    assert.equal(app.documentValue("#contact-outline-shape-select"), "rectangle");
    assert.equal(app.text("#save-state"), "Saved");
    assert.match(app.text("#editor-status"), /reverted/i);
...
test("constrained movement preserves metadata while every rotation mode updates sibling angles", async () => {
  ...
  await withEditor(async (app) => {
    app.setSvgGeometry("#editor-svg", { rect: { left: 0, top: 0, width: 120, height: 80 } });
    await app.click('[data-contact-key="a-piece-0"]');
    await drag(app, '[data-contact-key="a-piece-0"]', [{ x: 20, y: 20 }, { x: 25, y: 23 }]);
    await app.keyDown("body", "ArrowRight");
    await app.click("#contact-rotate-cw-button");
    await app.keyDown("body", "]");
