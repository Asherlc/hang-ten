...
def test_trango_rock_prodigy_natural_preserves_audited_topology_and_mirrors() -> None:
    ...
    assert board.facts["productURL"] == "https://trango.com/products/rock-prodigy-natural"
    assert board.facts["dimensions"] == "Each board: 7.5 × 6 × 1.5 in"
    assert board.facts["aspectRatio"] == pytest.approx(2.5, abs=1e-12)
    ...
    for stem, expected_metadata in EXPECTED_OPTIONAL_METADATA_BY_STEM.items():
        for side in ("left", "right"):
            ...
            actual_metadata = (
                ...
                else (depth_range.lower_bound, depth_range.upper_bound),
                hold.grip_type,
