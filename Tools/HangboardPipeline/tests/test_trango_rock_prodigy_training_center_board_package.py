...
def test_trango_rock_prodigy_training_center_audited_structure_and_geometry() -> None:
    ...
    assert board.facts["dimensions"] == "9.1 × 12.1 in"
    assert math.isclose(board.facts["aspectRatio"], 1.0, abs_tol=1e-12)
    ...
    assert holds["edge-large-vder-left"].grip_type == "openHand"
    assert holds["edge-large-vder-right"].grip_type == "openHand"
    ...
    assert holds["pocket-three-finger-slot-left"].finger_capacity == 3
    assert holds["pocket-three-finger-slot-right"].finger_capacity == 3
    assert holds["pocket-three-finger-slot-left"].grip_type == "threeFingerPocket"
    assert holds["pocket-three-finger-slot-right"].grip_type == "threeFingerPocket"
    assert holds["pocket-three-finger-slot-left"].features == (
        "shallowThreeFingerSlot",
    )
    assert holds["pocket-three-finger-slot-right"].features == (
        "shallowThreeFingerSlot",
    )
    assert all(
    ...
    assert all(
        holds[hold_id].grip_type == "twoFingerPocket"
        for hold_id in (
            "pocket-index-middle-deep-left",
            "pocket-index-middle-deep-right",
            "pocket-middle-ring-deep-left",
    ...
    assert all(
        holds[hold_id].features == ("deepTwoFingerPocket",)
        for hold_id in (
            "pocket-index-middle-deep-left",
            "pocket-index-middle-deep-right",
            "pocket-middle-ring-deep-left",
    ...
    assert all(
        holds[hold_id].features == ("twoFingerPocket",)
        for hold_id in (
            "pocket-index-middle-medium-shallow-left",
            "pocket-index-middle-medium-shallow-right",
            "pocket-middle-ring-shallow-left",
