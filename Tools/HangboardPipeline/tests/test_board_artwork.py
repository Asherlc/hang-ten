...
@pytest.mark.parametrize("missing", ["evidence.json", "semantics.json"])
...
def test_approved_package_rejects_invalid_semantics_and_symlinks(tmp_path: Path) -> None:
    module = load_board_catalog_module()
    root = _write_approved_package(tmp_path / "package")
...
def test_approved_package_rejects_obsolete_artwork_sidecar(tmp_path: Path) -> None:
    module = load_board_catalog_module()
    root = _write_approved_package(tmp_path / "package")
...
def test_approved_package_requires_exact_semantic_evidence_keys(tmp_path: Path) -> None:
    module = load_board_catalog_module()
    root = _write_approved_package(tmp_path / "package")
    evidence_path = root / "evidence.json"
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
