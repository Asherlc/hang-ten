...
from .semantic_holds import (
    CapturedSemanticGripProvider,
    PieceDescriptor,
    SemanticGripProvider,
)
...
def run_stage2_onboarding(
    ...
    *,
    ...
    raster = SimplifiedRaster(
        rgba, rgba[..., :3], np.zeros(rgba.shape[:2], dtype=np.uint8)
    )
    ...
    if (
        discovery_identity(first) != discovery_identity(second)
        or first.provider_response.raw_bytes != second.provider_response.raw_bytes
    ):
    ...
    try:
        ...
        if artifact_root.exists():
            raise FileExistsError(
                f"stage 2 artifact root already exists: {artifact_root}"
            )
...
def _validate_stage1(
    ...
    if not evidence.stage1_acceptance_path.is_file():
        raise FileNotFoundError(
            f"required Stage 1 acceptance is missing: {evidence.stage1_acceptance_path}"
        )
    if not evidence.stage1_rgba_path.is_file():
        raise FileNotFoundError(
            f"required Stage 1 RGBA is missing: {evidence.stage1_rgba_path}"
        )
    ...
    try:
        document = json.loads(raw_acceptance.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("Stage 1 acceptance is invalid JSON") from error
    expected_root_fields = {
        "accepted",
        "artifacts",
        "copiedAcceptedPngBytes",
        "pieces",
        "profile",
        "source",
        "stage",
        "stage0AcceptanceSha256",
        "stage0SelectedSourceId",
        "validation",
    }
    if (
    ...
    raw_rgba = evidence.stage1_rgba_path.read_bytes()
    if (
        not isinstance(artifacts, dict)
        or artifacts.get("automatedRgba") != evidence.stage1_rgba_path.name
        or not isinstance(validation, dict)
    ):
        raise ValueError("Stage 1 acceptance/RGBA contract mismatch")
    expected_file = (
        validation.get("pngFileSha256", {}).get("automatedRgba")
        if isinstance(validation.get("pngFileSha256"), dict)
        else None
    )
    if (
        expected_file != policy.rgba_file_sha256
        or sha256(raw_rgba).hexdigest() != policy.rgba_file_sha256
    ):
        raise ValueError("Stage 1 RGBA evidence identity mismatch")
    try:
        with Image.open(BytesIO(raw_rgba)) as image:
            if image.format != "PNG" or image.mode != "RGBA":
                raise ValueError("Stage 1 automated raster must be an RGBA PNG")
            rgba = np.asarray(image).copy()
    except OSError as error:
        raise ValueError("could not decode Stage 1 RGBA") from error
    if (
    ...
    if not isinstance(raw_pieces, list) or not raw_pieces:
        raise ValueError("Stage 1 piece metadata is missing")
    pieces: list[PieceDescriptor] = []
    for value in raw_pieces:
        if not isinstance(value, dict) or set(value) != {"id", "index", "material"}:
            raise ValueError("unsupported Stage 1 piece metadata")
...
def _validate_topology(discovery: HoldDiscovery) -> None:
    counts = {
        kind: sum(hold.grip_type == kind for hold in discovery.holds)
        for kind in ("pocket", "jug", "sloper")
    }
    if len(discovery.holds) != 22 or counts != {"pocket": 17, "jug": 2, "sloper": 3}:
        raise ValueError(
            "Stage 2 discovery did not produce the required 22-region topology"
        )
    if tuple(hold.id for hold in discovery.holds) != tuple(
        f"piece-01-hold-{index:02d}" for index in range(1, 23)
    ):
...
def _write_candidate_bundle(
    root: Path,
    rgba: np.ndarray,
    discovery: HoldDiscovery,
    stage1: Mapping[str, object],
    stage1_acceptance_raw_sha256: str,
    stage1_policy_id: str,
    profile: HoldDiscoveryProfile,
) -> None:
    (root / "stage-2-proposals.json").write_bytes(discovery.provider_response.raw_bytes)
    Image.fromarray(rgba, "RGBA").save(
        root / "stage-2-source-rgba.png", format="PNG", optimize=False, compress_level=9
    )
    Image.fromarray(discovery.label_raster, mode="I;16").save(
        root / "stage-2-labels.png", format="PNG", optimize=False, compress_level=9
    )
    Image.fromarray(discovery.candidate_evidence_rgb).save(
        root / "stage-2-candidates.png", format="PNG", optimize=False, compress_level=9
    )
    _write_json(root / "stage-2-auto-regions.json", discovery.regions_document())
    Image.fromarray(_labeled_trace(rgba, discovery)).save(
        root / "stage-2-auto-regions-labeled.png",
        format="PNG",
        optimize=False,
        compress_level=9,
    )
    ...
    hashes = {
        name: sha256((root / name).read_bytes()).hexdigest() for name in candidate_names
    }
    _write_json(
        root / "stage-2-candidate-hashes.json", {"files": hashes, "schemaVersion": 1}
    )
...
def _labeled_trace(rgba: np.ndarray, discovery: HoldDiscovery) -> np.ndarray:
    ...
    for index, hold in enumerate(discovery.holds):
        ...
        selected = hold.mask.astype(bool)
        blended[selected] = blended[selected] * 0.48 + color * 0.52
        boundary = selected & ~(cv2.erode(hold.mask, np.ones((3, 3), np.uint8)) > 0)
        blended[boundary] = (18, 18, 18)
    ...
    for index, hold in enumerate(discovery.holds, start=1):
        ...
        draw.ellipse(
            (x - tw / 2 - 3, y - th / 2 - 2, x + tw / 2 + 3, y + th / 2 + 2),
            fill=(255, 255, 255),
            outline=(10, 10, 10),
        )
