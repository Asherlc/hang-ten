...
def commercial_identity(asserted_name: str) -> CommercialIdentityAssertion:
    ...
    if (
        not isinstance(asserted_name, str)
        or not asserted_name
        or not all(character.isprintable() for character in asserted_name)
    ):
...
def run_generic_stage0(
    ...
    *,
    ...
    if profile.canonical_width <= 0 or not 1 <= profile.maximum_piece_count <= len(
        _COMPONENT_COLORS
    ):
    ...
    if (
        first.rgba.tobytes() != second.rgba.tobytes()
    ...
    try:
        ...
        _write_png(
            temporary_root / "stage-0-isolation.png",
            _isolation_overlay(rgb, first.labels, first.quadrilateral),
        )
        ...
        review = _review_image(
            rgb, first.overlay, first.rgba, identity, source.sanitized_locator
        )
...
def _decode_cached_rgb(raw: bytes, source: CachedSource) -> np.ndarray:
    ...
    if (
        rgb.shape[:2] != (source.height, source.width)
        or sha256(rgb.tobytes()).hexdigest() != source.decoded_rgb_sha256
    ):
...
def _register(rgb: np.ndarray, profile: GenericStage0Profile) -> _Registration:
    ...
    mask = (
        np.linalg.norm(lab - background, axis=2) > profile.background_tolerance
    ).astype(np.uint8)
    ...
    minimum_area = max(
        profile.minimum_component_area_ratio * height * width,
        profile.minimum_relative_component_area * largest_area,
    )
    retained = [
        label
    ...
    if not 1 <= len(retained) <= profile.maximum_piece_count:
    ...
    if not 1.5 <= ratio <= 12.0:
        raise ConversionError(
            "commercial product aspect ratio is outside Stage 0 limits"
        )
    output_height = round(profile.canonical_width * rectified.height / rectified.width)
    resized_rgb = cv2.resize(
        rectified.rgb,
        (profile.canonical_width, output_height),
    ...
    return _Registration(
        ...
        metadata={
            ...
            "quadrilateral": [
                [round(float(value), 6) for value in point] for point in quadrilateral
            ],
        },
        overlay=overlay,
    )
...
def _order_corners(points: np.ndarray) -> np.ndarray:
    ...
    if np.linalg.norm(edges[1]) + np.linalg.norm(edges[3]) > np.linalg.norm(
        edges[0]
    ) + np.linalg.norm(edges[2]):
    ...
    top, bottom = sorted(
        ((cyclic[0], cyclic[1]), (cyclic[2], cyclic[3])),
...
def _isolation_overlay(
    rgb: np.ndarray, labels: np.ndarray, quadrilateral: np.ndarray
) -> np.ndarray:
    overlay = rgb.copy()
    for index, color in enumerate(_COMPONENT_COLORS, start=1):
        selected = labels == index
        overlay[selected] = np.rint(
            0.55 * overlay[selected] + 0.45 * np.asarray(color)
        ).astype(np.uint8)
    cv2.polylines(
        overlay,
        [np.rint(quadrilateral).astype(np.int32)],
        True,
        (24, 24, 24),
        2,
        cv2.LINE_AA,
...
def _review_image(
    ...
    draw.text(
        (50, 35),
        f"Stage 0 review: {identity.asserted_name}",
        fill=(30, 30, 30),
        font=font,
    )
    ...
    draw.text(
        (1150, 95),
        "Isolation and registration quadrilateral",
        fill=(30, 30, 30),
        font=font,
    )
    ...
    draw.text(
        (600, 825),
        f"Registered RGBA ({rgba.shape[1]} px wide)",
        fill=(30, 30, 30),
        font=font,
    )
