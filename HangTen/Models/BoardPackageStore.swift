...
struct BoardPackageStore {
    ...
    private static func loadPackage(
        ...
        do {
            ...
            try rawDocument.validateDuplicateKeysOutsideOrientations()
            try rawDocument.validateModelOrientationMemberOrder()
    ...
    private static func makeModelSuspension(
        ...
        case .pairedLeadCord(let pairedLead):
            return try makePairedLeadCordSuspension(pairedLead, descriptor: descriptor, positionIDs: positionIDs, boardID: boardID)
    ...
    private static func makePairedLeadCordSuspension(
        _ document: BoardPackagePairedLeadCordSuspensionDocument,
        descriptor: BoardModelDescriptor,
        positionIDs: Set<String>,
        boardID: String
    ) throws -> BoardModelSuspension {
        ...
        for attachment in document.attachments {
            guard let node = nodesByID[attachment.nodeID], node.role == .body || node.role == .attachment else {
                throw BoardPackageStoreError.invalidPackage(boardID: boardID, reason: "paired lead attachment node must be a body or attachment node")
            }
            guard attachment.pointInModel.count == 3,
                  attachment.pointInModel.allSatisfy(\.isFinite),
                  zip(attachment.pointInModel, descriptor.modelBounds.minimum).allSatisfy({ $0 >= $1 }),
        ...
        guard anchorPosition.allSatisfy(\.isFinite) else {
            throw BoardPackageStoreError.invalidPackage(boardID: boardID, reason: "pairedLeadCord anchor must be finite")
        }
        var poses: [String: BoardModelCanonicalPose] = [:]
        for (positionID, pose) in document.canonicalPoses {
    ...
    private static func makeTwoBranchSuspension(
        ...
        for passage in passages {
            ...
            guard boreLength.isFinite, !throughBore || boreLength > 1e-7 else {
        ...
        guard document.branches.allSatisfy({
            ...
            (!throughBore || ($0.entryContactPoints.count >= 1 &&
        ...
        for (positionID, pose) in document.canonicalPoses {
            ...
            for (branchIndex, branch) in document.branches.enumerated() {
                ...
                guard rigidLength.isFinite, firstDistance.isFinite, lastDistance.isFinite,
                      rigidLength > 1e-7, firstDistance > 1e-7, lastDistance > 1e-7,
        ...
        return .twoBranchCord(BoardModelTwoBranchSuspension(
            passages: BoardModelPassagePairs(
                left: document.passages.left.map(\.modelValue),
                right: document.passages.right.map(\.modelValue)
            ),
...
private indirect enum BoardPackageRawJSONValue: Equatable {
    ...
    func validateDuplicateKeysOutsideOrientations() throws {
        func visit(_ value: BoardPackageRawJSONValue, isOrientation: Bool) throws {
            switch value {
            case .object(let members):
                if !isOrientation, Set(members.map(\.name)).count != members.count {
                    throw BoardPackageRawJSONError.invalid
                }
                for member in members {
                    try visit(member.value, isOrientation: isOrientation || member.name == "orientation")
                }
            case .array(let values):
                for value in values { try visit(value, isOrientation: isOrientation) }
            default: break
            }
        }
        try visit(self, isOrientation: false)
    }

    func validateModelOrientationMemberOrder() throws {
        guard case .object(let root) = self else {
            throw BoardPackageRawJSONError.invalid
        }
        guard let presentationsValue = root.value(named: "presentations") else { return }
        guard case .array(let presentations) = presentationsValue else {
            throw BoardPackageRawJSONError.invalid
        }
        for presentation in presentations {
            guard case .object(let presentationMembers) = presentation,
                  case .object(let media)? = presentationMembers.value(named: "media"),
                  case .object(let orientation)? = media.value(named: "orientation") else { continue }
            guard orientation.names == ["pivot", "rotations"],
                  case .object(let rotations)? = orientation.value(named: "rotations") else {
                throw BoardPackageRawJSONError.orientation("orientation must contain canonical pivot and rotations members")
            }
            guard Set(rotations.names).count == rotations.names.count else {
                throw BoardPackageRawJSONError.orientation("orientation rotations must not contain duplicate position IDs")
            }
            guard rotations.names == rotations.names.sorted() else {
                throw BoardPackageRawJSONError.orientation("orientation rotations must be sorted by position ID")
            }
        }
    }
...
struct BoardPackagePassageDocument: Decodable, Equatable {
    ...
    var modelValue: BoardModelPassage {
        isThroughBore
            ? BoardModelPassage(id: id, nodeID: nodeID, entryPointInModel: entryPointInModel, exitPointInModel: exitPointInModel, provenance: provenance)
            : BoardModelPassage(id: id, nodeID: nodeID, pointInModel: entryPointInModel, provenance: provenance)
    }

    private enum CodingKeys: String, CodingKey { case id, nodeID, pointInModel, entryPointInModel, exitPointInModel, provenance }
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        isThroughBore = !container.contains(.pointInModel)
        try decoder.rejectUnknownKeys(Set(["id", "nodeID", "provenance"] + (isThroughBore ? ["entryPointInModel", "exitPointInModel"] : ["pointInModel"])))
        id = try container.decode(String.self, forKey: .id)
        nodeID = try container.decode(String.self, forKey: .nodeID)
        entryPointInModel = try container.decode([Double].self, forKey: isThroughBore ? .entryPointInModel : .pointInModel)
        exitPointInModel = try isThroughBore ? container.decode([Double].self, forKey: .exitPointInModel) : entryPointInModel
        provenance = try container.decode(String.self, forKey: .provenance)
    }
}
...
struct BoardPackageCordBranchDocument: Decodable, Equatable {
    ...
    init(from decoder: Decoder) throws {
        ...
        let container = try decoder.container(keyedBy: CodingKeys.self)
        hasContactRoute = container.contains(.entryContactPoints) || container.contains(.exteriorContactPoints) || container.contains(.exitContactPoints)
        id = try container.decode(String.self, forKey: .id)
        passageIDs = try container.decode([String].self, forKey: .passageIDs)
        entryContactPoints = try hasContactRoute ? container.decode([[Double]].self, forKey: .entryContactPoints) : []
        exteriorContactPoints = try hasContactRoute ? container.decode([[Double]].self, forKey: .exteriorContactPoints) : []
        exitContactPoints = try hasContactRoute ? container.decode([[Double]].self, forKey: .exitContactPoints) : []
