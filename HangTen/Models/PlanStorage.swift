...
struct SemanticHoldMappingDefinition: Codable, Hashable {
    ...
    func holdTarget() -> HoldTarget {
        if !holdIDs.isEmpty {
            return HoldTarget(holdIDs: holdIDs, kind: nil)
        }
        if let kind {
            return HoldTarget(holdIDs: [], kind: kind)
        }
        return HoldTarget(holdIDs: [], kind: nil)
    }
}
                ...
                for (stepIndex, stepDefinition) in block.steps.enumerated() {
                    ...
                    steps.append(
                        WorkoutStep(
                            ...
                            activeDuration: stepDefinition.activeDuration
                        )
                    )
                }
            }
...
enum BuiltInPlanLibraryDefinition {
    ...
    private static let semanticHoldIDs: [String: [String]] = [
        ...
        "deep-four-finger-pockets": ["pocket-4-deep-left", "pocket-4-deep-right"],
        "three-finger-pockets": ["pocket-3-shallow-left", "pocket-3-shallow-right"],
        "center-sloper": ["sloper-center"],
        "lower-center-sloper": ["sloper-center-lower"]
    ]
    ...
    private static func makeDocument() -> PlanLibraryDefinition {
        ...
        return PlanLibraryDefinition(
            metadata: PlanLibraryMetadata(
                ...
                notes: [
                    "Built-in routines are source-linked timer and hold-cue translations.",
    ...
    private static func stepDefinition(from step: WorkoutStep, id: String? = nil) -> WorkoutStepDefinition {
        WorkoutStepDefinition(
            ...
            activeDuration: step.activeDurationOverride
        )
    }
...
enum PlanCatalog {
    ...
    static let metoliusTenMinute = required("metolius.compact-ii.ten-minute")
