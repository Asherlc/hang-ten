...
struct MotherboardWorkoutRecorder {
    private struct StepState {
        ...
        var plannedEnd: TimeInterval {
            timeOffset + stepStartElapsed + plannedActiveDuration
        }
    }
    ...
    mutating func consume(
        ...
        let measurementTime = measurement.timestamp.timeIntervalSince1970
        currentLoadKGF = measurement.aggregateLoadKGF
        currentStepID = stepID

        var state = state(for: stepID, plannedActiveDuration: plannedActiveDuration, measurementTime: measurementTime, workoutElapsed: workoutElapsed)
        guard isActive else {
            states[stepID] = state
            return
        }
        ...
        currentPeakLoadKGF = max(currentPeakLoadKGF ?? measurement.aggregateLoadKGF, measurement.aggregateLoadKGF)

        let time = state.clippedTime(measurementTime)
    ...
    private mutating func state(
        ...
        let state = StepState(
            ...
            timeOffset: measurementTime - workoutElapsed,
            stepStartElapsed: workoutElapsed
        )
        states[stepID] = state
    ...
    private mutating func openInterval(in state: inout StepState, at time: TimeInterval) {
        ...
        if let previous = state.intervals.last,
