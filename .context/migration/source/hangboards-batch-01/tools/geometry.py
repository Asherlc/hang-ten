...
def body_field(x,y,z,cfg):
    ...
    for cut in cfg.get('bodyCuts',[]):
        ...
        elif cut['kind']=='archedUndercut':
            # Manually authored rolled-lip undercut. The central circular arch is
            # a photographed silhouette feature, not extracted from image pixels.
            dx=x-cut.get('x',0.)
            cap=cut['baseZ']+np.sqrt(np.maximum(cut['archRadius']**2-dx**2,0.))
            void=np.maximum(z-cap,cut['bottomZ']-z)
            void=smax(void,y-cut['floorY'],cut.get('floorRound',2.))
            a,b=cut['xRange']; void=smax(void,np.maximum(a-x,x-b),cut.get('sideRound',2.))
            body=smax(body,-void,cut.get('rim',2.5))
...
def classify(mesh,cfg):
    ...
    for surface in cfg.get('surfaceContacts',[]):
        ...
        if surface.get('archBoundary'):
            a=surface['archBoundary']
            bottom=a['baseZ']+np.sqrt(np.maximum(a['radius']**2-(x-a.get('x',0.))**2,0.))
            mask &= z > bottom-a.get('band',4.)
...
def build(cfg):
    ...
    # A declared sub-voxel phase makes the sampling lattice explicit. It does
    # not move the authored solid. Topology must still be independently checked
    # after decimation; a sampling phase is not a topology guarantee.
    axes=[a+phase for a,phase in zip(axes,cfg.get('gridPhaseMm',[0.,0.,0.]))]
