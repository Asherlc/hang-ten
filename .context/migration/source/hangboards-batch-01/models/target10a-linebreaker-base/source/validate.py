...
def validate(path,cfg,hold_map):
    ...
    for p in cfg.get('pockets',[]):
        for offset in [-.17,.17]:
            ...
            ok=hitY is not None and abs(hitY-p['floorY'])<1.5
            rayresults.append({'holdId':p.get('holdId'),'xOffsetFraction':offset,'expectedFloorYmm':p['floorY'],'actualFirstHitYmm':hitY,'pass':bool(ok)})
