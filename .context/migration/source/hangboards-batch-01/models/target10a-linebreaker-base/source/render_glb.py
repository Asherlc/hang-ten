...
def render(path,out,views=None,title=None):
    ...
    with tempfile.TemporaryDirectory(prefix='hangboard-render-') as td:
        ...
        rw=vtk.vtkRenderWindow(); rw.SetOffScreenRendering(1);rw.SetSize(1600,900);rw.SetMultiSamples(8);rw.AddRenderer(ren)
        ...
        for filename,label,view,target,scale in views:
            ...
            im=Image.open(png).convert('RGB'); canvas=Image.new('RGB',(1600,1000),'white');canvas.paste(im,(0,60));draw=ImageDraw.Draw(canvas)
            try:
        ...
        result={'reader':'VTK vtkGLTFImporter','version':vtk.vtkVersion.GetVTKVersion(),'assetSha256':digest,'isolatedDirectoryContents':['asset.glb'],'actorCount':len(ac),'bounds':list(bounds),'renders':records}
