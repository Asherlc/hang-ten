...
def front_profile_clearance(y,z,poly):
    """Unsigned clearance from the authored non-rear side boundary.
    The rear plane is intentionally omitted so the cavity stays OPEN at the rear.
    This geometrical wall constraint prevents the rear cut from puncturing a ledge.
    """
    shape=np.broadcast_shapes(np.shape(y),np.shape(z))
    d=np.full(shape,np.inf,dtype=np.float64)
    a=np.asarray(poly,float)
    for p,q in zip(a,np.roll(a,-1,axis=0)):
        if abs(p[0])<1e-9 and abs(q[0])<1e-9: continue
        ey,ez=q-p; den=ey*ey+ez*ez
        if den<=1e-15: raise ValueError('Zero-length authored profile segment')
        u=y-p[0];v=z-p[1];t=np.clip((u*ey+v*ez)/den,0,1)
        d=np.minimum(d,(u-t*ey)**2+(v-t*ez)**2)
    return np.sqrt(d)
...
def evaluate(x,y,z,cfg):
    ...
    if cfg.get('rearBays'):
        section=polygon_sdf(y,z,cfg['rearProfileYZ'])
        section=np.maximum(section,cfg['estimatedMinimumWallMm']-front_profile_clearance(y,z,cfg['profileYZ']))
        for bay in cfg['rearBays']:
            opening=rrect(x,z,bay['x'],bay['z'],bay['width'],bay['height'],bay['radius'])
            # Bosses are physical returns into the two openings, NOT floating inserts.
            for boss in cfg['rearBosses']:
                b=rrect(x,z,boss['x'],boss['z'],boss['width'],boss['height'],boss['radius'])
                opening=smax(opening,-b,.65)
            void=smax(opening,section,1.2)
            field=smax(field,-void,.7)
...
def classify(mesh,cfg):
    ...
    for surface in cfg.get('surfaceContacts',[]):
        ...
        if surface.get('outerProfileOnly'):
            mask &= np.abs(polygon_sdf(y,z,cfg['profileYZ']))<1.15
            # Exclude interior cavity faces and the planar rear from the contact patch.
            mask &= y<-.2
        if surface.get('frontJug'):
            mask &= ((y<-46) | (z>129))
        labels[mask]=len(ids)
...
def build(cfg):
    ...
    # The isosurface/decimation combination can leave an opposite-winding coincident
    # triangle pair: a zero-volume sheet, not physical material. Remove both, never
    # merely one side. Same-winding duplicates are a build error and are not hidden.
    sf=np.sort(mesh.faces,axis=1)
    _, inverse, counts=np.unique(sf,axis=0,return_inverse=True,return_counts=True)
    opposed_cleanup=0
    if np.any(counts>1):
        for group in np.flatnonzero(counts>1):
            fids=np.flatnonzero(inverse==group)
            if len(fids)!=2 or np.dot(mesh.face_normals[fids[0]],mesh.face_normals[fids[1]])>-.999:
                raise ValueError('Unexpected duplicate physical surface')
        remove=counts[inverse]>1;opposed_cleanup=int(remove.sum())
        mesh.update_faces(~remove);mesh.remove_unreferenced_vertices()
    if not mesh.is_watertight:
        raise ValueError('Assembled surface must be closed before contact partition/export')
    ...
    # Exactly planar triangles must not interpolate curved bore-rim normals across
    # a long front-face triangle. Preserve the authored round surfaces elsewhere.
    coords=mesh.vertices[mesh.faces]
    for axis in range(3):
        flat=(np.ptp(coords[:,:,axis],axis=1)<.002) & (np.abs(mesh.face_normals[:,axis])>.99999)
        exact=np.zeros((int(flat.sum()),3));exact[:,axis]=np.sign(mesh.face_normals[flat,axis])
        corner_normals[flat]=exact[:,None,:]
    ...
    return mesh,parts,labels,dict(rawTriangles=rawcount,finalTriangles=len(mesh.faces),voxelMm=pitch,
                                 ...
                                 contactIds=ids,partitionCleanup=partition_cleanup,opposingZeroVolumeTrianglesRemoved=opposed_cleanup)
