...
- Use primary manufacturer evidence first. Before Astra resumes, each Stage 0 packet must retain at least two complete exact-revision images per board: one manufacturer-published image and, where available, a materially different oblique/side/back/profile image. Record local path, SHA-256, pixel dimensions, retrieval date/locale, angle label, page linkage, source tier, and angle-specific geometric support limits. Authorized commerce evidence can fill a documented first-party gap only and must be labeled commerce-gap; search snippets, reviews, forums, crops, duplicates, and AI summaries are discovery/corroboration, never sole authority.
...
- [ ] Write negative packet fixtures at `.context/shaky-rat-beastmaker-1000/rejected-snippet-packet.json` and `.context/shaky-rat-metolius-wood-grips-compact-ii/rejected-retailer-packet.json`; run `rtk proxy python3 -B Tools/HangboardModels/validate_evidence_packet.py .context/shaky-rat-beastmaker-1000/rejected-snippet-packet.json` and the equivalent Compact path. Require exit 2: Beastmaker must reach and reject the manufacturer search-result URL rule after its source bytes/hash validate; Compact must reject its missing retained commerce snapshot.
- [ ] Gather and hash actual manufacturer pages/media for both boards, retaining at least two complete exact-revision images per board with front plus a materially different angle where available. Record pixel dimensions, retrieval date/locale, angle labels, page linkage, and what each angle can/cannot establish. Record authorized-commerce gaps only when needed and label them; validate both approved packets with two explicit CLI invocations, and require exit 0 for each.
- [ ] Record Beastmaker’s sourced 580 × 150 mm face, qualified 58 mm depth ruling and current Tulipwood `5 mm` conflict; record Compact’s current Metolius evidence, 610 × 157 mm dimensions, and lower Compact diagram’s #2 56 mm flat-sloper and #9 56 mm round-sloper callouts. The diagram does not establish Compact body/display thickness: retain physical body Z/depth as source-unresolved and label any existing model depth estimated authored geometry. Keep all 22/19 stable IDs and explicitly omit hardware.
