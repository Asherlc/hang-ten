...
The solver uses the existing fixed 32 samples, gravity vector, `1e-6 m` bisection tolerance, and `1e-5 m` taut tolerance. Each branch is the ordered continuous path `anchor -> passage[0] -> passage[1] -> anchor`: solve the two free anchor-to-passage catenaries, preserve the modeled interior passage span, and validate joins/length/clearance inputs without inventing knot geometry.
