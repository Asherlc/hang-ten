...
Make `review_cli.main(["inspect", "--run", str(run)])` return `0` and print an object containing `state`, `nextAction`, relative artifact paths, and hashes. Add only the `inspect` dispatch to `scripts/hangboard-tools.sh` in this task; later tasks add their own wrapper commands. Return `2` for invalid arguments and `3` for filesystem or artifact errors, matching `onboard_cli.py`.
