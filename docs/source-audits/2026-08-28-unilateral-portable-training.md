...
| presentations and every path | The package now uses the exact current-revision official `Primary`, `Back`, and `Side` manufacturer packshots documented in the [2026-08-29 presentation-asset audit](2026-08-29-official-portable-presentation-assets.md). An operator deliberately drew and reviewed each canonical contact path against those packshots: pills for the measured recess contacts and outer-rim jug, and a rounded rectangle for the side-body pinch. No coordinates, contours, masks, or hold proposals were extracted from image pixels, and no registration, alignment, segmentation, tracing, vectorization, automatic cropping, contour extraction, generated mask, or proposal/refine workflow was used. The saved paths are the rendering, highlighting, and hit-testing source of truth. |
