...
The generic fallback adapter rejects manufacturer payloads shorter than 12 bytes and reads payload indexes relative to the payload collection start, so sliced `Data` values decode the same way as zero-indexed buffers.
