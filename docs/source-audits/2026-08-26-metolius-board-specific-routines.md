...
Checked 2026-09-15 against Metolius' primary guides: [Contact Training Guide](https://www.metoliusclimbing.com/pages/contact-training-guide) and [Simulator 3D Training Guide](https://www.metoliusclimbing.com/pages/simulator-3d-training-guide).
...
The factual board inventory is separately audited in [the Metolius
board-package audit](2026-08-12-metolius-board-packages.md#contact-reviewed-label-map).
Routine targets deliberately contain no stable IDs, hold numbers, or visual
frames. A bilateral jug requirement is resolved from the default presentation's
outermost factual left/right pair; a single requirement resolves one stable
geometry-selected factual contact. The guide's parenthetical numbers remain
verbatim in task text. When a number and its adjective conflict with the board
inventory, do not relabel another contact merely to manufacture a match.
...
Board classification: `metolius.simulator-3d` (package slug `metolius-simulator-3d`).
The semantic target key is: outer jugs (1) → `kind: jug`,
`.bilateralPair`; center jug (14) → `kind: jug`, `.single` (the single-contact
resolver selects the centered factual jug); flat slopers (2) → `kind: sloper`,
`shape: flat`, `.bilateralPair`; round slopers (3) → `kind: sloper`,
`shape: round`, `.single`; and numbered edges/pockets → their documented kind,
depth, and finger-capacity predicates. These requirements contain no contact
IDs: resolution uses the factual #2/#3 shapes and default geometry.
...
| 2 | 15 s dead hang + one pull-up outer jugs (1) → `jug-1-left/right` | 20 s dead hang flat slopers (2); 3 pull-ups flat slopers → semantic flat-sloper pair | 5 offset pull-ups pockets (15 & 12), **reverse holds repeat** → `pocket-15-center`; `pocket-12-left/right` |
...
| 4 | 15 s dead hang extra deep 3 finger pockets (9) → `pocket-9-left/right` | 15 s dead hang flat slope (2); 15 s dead hang round slopers (3) → semantic flat pair; semantic round sloper | 5 offset pull-ups round sloper (3) & deep pocket (4), **reverse holds repeat** → semantic round sloper; `pocket-4-left/right` |
| 5 | 12 s dead hang flat slopers (2); 5 knee raises outer jugs (1) → semantic flat pair; `jug-1-left/right` | 20 s offset hang jug (1) & shallow pocket (17), **reverse holds — repeat** → `jug-1-left/right`; `pocket-17-center` | 10 s dead hang x-shallow edges (11), **staying on**; campus to three finger pockets (9); campus to shallow edges (6); campus to flat slopers (2); hold 15 s → `edge-11-left/right`; `pocket-9-left/right`; `edge-6-left/right`; semantic flat pair |
| 6 | 16 s offset hang (8 s per side) deep pocket (15) & shallow edge (5) → `pocket-15-center`; `edge-5-left/right` | 15 s offset hang pockets (4 & 9), **reverse holds and repeat** → `pocket-4-left/right`; `pocket-9-left/right` | 15 s one-arm hang round sloper (3); rest 10 s; **repeat other arm** → semantic round sloper |
| 7 | 3 pull-ups outer jugs (1) → `jug-1-left/right` | 4 pull-ups medium edges; 10 knee raises any holds → `edge-5-left/right`; any Simulator 3D hold | 5 L-sit pull-ups (bend knees if you have to), jugs (1); 20 s bent-arm hang (elbows @ 90), deep two finger pockets (12) → `jug-1-left/right`; `pocket-12-left/right` |
| 8 | 8 s bent-arm hang (elbows @ 90), round slopers (3) → semantic round sloper | 30 s dead hang deep pockets (7) → `edge-7-left/right` | 20 s slightly bent-arm hang shallow 3 finger pocket (8), **stay on**; bump to x-deep three finger pockets (position 9); 25 s dead hang → `pocket-8-left/right`; `pocket-9-left/right` |
...
| 10 | Dead hang **to failure**, any holds → any Simulator 3D hold | 5 pull-ups deep edges (7), **without dropping off**, bump up to round slopers (3) & dead hang **till failure** → `edge-7-left/right`; semantic round sloper | 8 fast pull-ups jugs (1) (**keeping form perfect**); dead hang round sloper **to failure** (“fighting hard!”) → `jug-1-left/right`; semantic round sloper |
