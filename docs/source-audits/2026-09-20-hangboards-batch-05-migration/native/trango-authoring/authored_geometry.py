# Deliberate derivative: stricter analytic height error (0.025mm), and selected
# non-circular cubic lip section. Original geometry.py is retained unchanged.
...
class Unit:
    ...
    def cavity(self,c:dict,through=None,*,base_y=None,register=True):
        ...
        slope=c.get('slope',0)
        dips=c.get('dips', [c['dip']] if c.get('dip') else [])
        def warp(q):
            x,z=q
            for dip in dips:
                if z<cz:z-=dip['amount']*band(x,dip['x0'],dip['x1'],dip.get('blend',2.5))*min(1,(cz-z)/(h*.3))
        ...
        levels=[('front',0,0)]
        if c.get('lipProfile')=='authored-noncircular':
            for t in np.linspace(.125,1,8):levels.append(('front',rr*(1-(1-t)**3),rr*t*t))
        else:
